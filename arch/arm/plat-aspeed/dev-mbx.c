...
#if defined(CONFIG_AST_COLDFIRE_MBX) || defined(CONFIG_AST_COLDFIRE_MBX_MODULE)
static struct resource ast_cf_mbx_resource[] = {
	[0] = {
		.start = AST_SRAM_BASE,
...
static u64 ast_cf_mbx_dma_mask = 0xffffffffUL;

static struct platform_device ast_cf_mbx_device = {
	.name	= "ast-cf-mbx",
	.id = 0,
	.dev = {
		.dma_mask = &ast_cf_mbx_dma_mask,
		.coherent_dma_mask = 0xffffffff,
	},
	.resource = ast_cf_mbx_resource,
	.num_resources = ARRAY_SIZE(ast_cf_mbx_resource),
};

void __init ast_add_device_cf_mbx(void)
{
	platform_device_register(&ast_cf_mbx_device);
}
#else
void __init ast_add_device_cf_mbx(void) {}
#endif
