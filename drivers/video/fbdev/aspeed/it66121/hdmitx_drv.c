...
//HDMITXDEV hdmiTxDev[HDMITX_MAX_DEV_COUNT]; //ryan remove to driver data 
...
// Function Prototype
//////////////////////////////////////////////////////////////////////
void hdmitx_LoadRegSetting(struct i2c_client *client, RegSetEntry table[]);
...
void HDMITX_InitTxDev(HDMITXDEV *pInstance)
{
	...
	{
		cat66121_hdmi->hdmiTxDev = *pInstance ;
	}
}
#endif
void InitHDMITX(struct i2c_client *client)
{
	struct cat66121_hdmi_pdata *cat66121_hdmi = i2c_get_clientdata(client);

    hdmitx_LoadRegSetting(client, HDMITX_Init_Table);
//    HDMITX_WriteI2C_Byte(client, REG_TX_PLL_CTRL,0xff);
    cat66121_hdmi->hdmiTxDev.bIntPOL = (cat66121_hdmi->hdmiTxDev.bIntType&B_TX_INTPOL_ACTH)?TRUE:FALSE ;
    ...
    cat66121_hdmi->hdmiTxDev.RCLK = CalcRCLK();
#endif
    hdmitx_LoadRegSetting(client, HDMITX_DefaultVideo_Table);
    hdmitx_LoadRegSetting(client, HDMITX_SetHDMI_Table);
    hdmitx_LoadRegSetting(client, HDMITX_DefaultAVIInfo_Table);
    hdmitx_LoadRegSetting(client, HDMITX_DeaultAudioInfo_Table);
    hdmitx_LoadRegSetting(client, HDMITX_Aud_CHStatus_LPCM_20bit_48Khz);
    hdmitx_LoadRegSetting(client, HDMITX_AUD_SPDIF_2ch_24bit);
//  HDMITX_PowerDown(client);
    ...
    DumpHDMITXReg();
	HDMITX_WriteI2C_Byte(client, 0x63, 0x00);
	HDMITX_WriteI2C_Byte(client, 0x6A, 0xFF);
	
}

BOOL getHDMITX_LinkStatus(struct i2c_client *client)
{
    if(B_TX_RXSENDETECT & HDMITX_ReadI2C_Byte(client, REG_TX_SYS_STATUS))
    {
        if(0==HDMITX_ReadI2C_Byte(client, REG_TX_AFE_DRV_CTRL))
        {
...
BYTE CheckHDMITX(struct i2c_client *client, BYTE *pHPD,BYTE *pHPDChange)
{
	struct cat66121_hdmi_pdata *cat66121_hdmi = i2c_get_clientdata(client);

    BYTE intdata1,intdata2,intdata3,sysstat;
    BYTE  intclr3 = 0 ;
    BYTE PrevHPD = cat66121_hdmi->hdmiTxDev.bHPD ;
    BYTE HPD ;
    sysstat = HDMITX_ReadI2C_Byte(client, REG_TX_SYS_STATUS);
    ...
    if(sysstat & B_TX_INT_ACTIVE)
    {
        ...
        intdata1 = HDMITX_ReadI2C_Byte(client, REG_TX_INT_STAT1);
        ...
        if(intdata1 & B_TX_INT_AUD_OVERFLOW)
        {
            ...
            HDMITX_OrReg_Byte(client, REG_TX_SW_RST,(B_HDMITX_AUD_RST|B_TX_AREF_RST));
            HDMITX_AndReg_Byte(client, REG_TX_SW_RST,~(B_HDMITX_AUD_RST|B_TX_AREF_RST));
		...
		if(intdata1 & B_TX_INT_DDC_BUS_HANG)
		{
            ...
            if(cat66121_hdmi->hdmiTxDev.bAuthenticated)
            {
                ...
                #ifdef SUPPORT_HDCP
                hdmitx_hdcp_ResumeAuthentication(client);
                #endif
            }
		}
		if(intdata1 & (B_TX_INT_HPD_PLUG/*|B_TX_INT_RX_SENSE*/))
		{
            ...
            {
                /*
                HDMITX_WriteI2C_Byte(client, REG_TX_SW_RST,B_TX_AREF_RST|B_HDMITX_VID_RST|B_HDMITX_AUD_RST|B_TX_HDCP_RST_HDMITX);
                delay1ms(1);
                HDMITX_WriteI2C_Byte(client, REG_TX_AFE_DRV_CTRL,B_TX_AFE_DRV_RST|B_TX_AFE_DRV_PWD);
                */
                //HDMITX_DEBUG_PRINTF(("Unplug,%x %x\n",(int)HDMITX_ReadI2C_Byte(client, REG_TX_SW_RST),(int)HDMITX_ReadI2C_Byte(client, REG_TX_AFE_DRV_CTRL)));
            }
		}
        ...
        intdata2 = HDMITX_ReadI2C_Byte(client, REG_TX_INT_STAT2);
		...
		if(intdata2 & B_TX_INT_AUTH_DONE)
		{
            ...
            HDMITX_OrReg_Byte(client, REG_TX_INT_MASK2,(BYTE)B_TX_AUTH_DONE_MASK);
            //cat66121_hdmi->hdmiTxDev.bAuthenticated = TRUE ;
		...
		if(intdata2 & B_TX_INT_AUTH_FAIL)
		{
			...
			hdmitx_AbortDDC(client);   // @emily add
            //hdmitx_hdcp_ResumeAuthentication();
        }
        #endif // SUPPORT_HDCP

#if 1
		intdata3 = HDMITX_ReadI2C_Byte(client, REG_TX_INT_STAT3);
		...
		if(intdata3 & B_TX_INT_VIDSTABLE)
		{
			...
			{
				hdmitx_FireAFE(client);
			}
		}
#endif
        intdata3= HDMITX_ReadI2C_Byte(client, 0xEE);
        if( intdata3 )
        {
            HDMITX_WriteI2C_Byte(client, 0xEE,intdata3); // clear ext interrupt ;
	...
	intclr3 = (HDMITX_ReadI2C_Byte(client, REG_TX_SYS_STATUS))|B_TX_CLR_AUD_CTS | B_TX_INTACTDONE ;
	HDMITX_WriteI2C_Byte(client, REG_TX_SYS_STATUS,intclr3); // clear interrupt.
	HDMITX_WriteI2C_Byte(client, REG_TX_INT_CLR0,0xFF);
	HDMITX_WriteI2C_Byte(client, REG_TX_INT_CLR1,0xFF);
	intclr3 &= ~(B_TX_INTACTDONE);
	HDMITX_WriteI2C_Byte(client, REG_TX_SYS_STATUS,intclr3); // INTACTDONE reset to zero.
    }
    //
    // else
    // {
    ...
    cat66121_hdmi->hdmiTxDev.bHPD = HPD ;
    return HPD ;
}

void HDMITX_PowerOn(struct i2c_client *client)
{
	PowerStatus = TRUE;
    hdmitx_LoadRegSetting(client, HDMITX_PwrOn_Table);
}

void HDMITX_PowerDown(struct i2c_client *client)
{
	PowerStatus = FALSE;
    hdmitx_LoadRegSetting(client, HDMITX_PwrDown_Table);
}
...
void setHDMITX_AVMute(struct i2c_client *client, BYTE bEnable)
{
    Switch_HDMITX_Bank(client, 0);
    HDMITX_SetI2C_Byte(client, REG_TX_GCP,B_TX_SETAVMUTE, bEnable?B_TX_SETAVMUTE:0 );
    HDMITX_WriteI2C_Byte(client, REG_TX_PKT_GENERAL_CTRL,B_TX_ENABLE_PKT|B_TX_REPEAT_PKT);
}
...
void hdmitx_LoadRegSetting(struct i2c_client *client, RegSetEntry table[])
{
    ...
    for( i = 0 ;  ; i++ )
    {
        ...
        {
            HDMITX_DEBUG_PRINTF2(("HDMITX_WriteI2C_Byte(client, %02x,%02x)\n",(int)table[i].offset,(int)table[i].OrMask));
            HDMITX_WriteI2C_Byte(client, table[i].offset,table[i].OrMask);
        }
        else
        {
            HDMITX_DEBUG_PRINTF2(("HDMITX_SetI2C_Byte(client, %02x,%02x,%02x)\n",(int)table[i].offset,(int)table[i].invAndMask,(int)table[i].OrMask));
            HDMITX_SetI2C_Byte(client, table[i].offset,table[i].invAndMask,table[i].OrMask);
        }
    }
}
...
BOOL getHDMITX_EDIDBlock(struct i2c_client *client, int EDIDBlockID,BYTE *pEDIDData)
{
	if(!pEDIDData)
	{
		return FALSE ;
	}
    if(getHDMITX_EDIDBytes(client, pEDIDData,EDIDBlockID/2,(EDIDBlockID%2)*128,128) == ER_FAIL)
    {
        return FALSE ;
    }
    return TRUE ;
...
SYS_STATUS getHDMITX_EDIDBytes(struct i2c_client *client, BYTE *pData,BYTE bSegment,BYTE offset,SHORT Count)
{
...
//        HDMITX_DEBUG_PRINTF(("getHDMITX_EDIDBytes(): Invallid pData pointer %08lX\n",(ULONG)pData));
    ...
    if(HDMITX_ReadI2C_Byte(client, REG_TX_INT_STAT1) & B_TX_INT_DDC_BUS_HANG)
    {
    ...
    while(RemainedCount > 0)
    {
        ...
        HDMITX_WriteI2C_Byte(client, REG_TX_DDC_MASTER_CTRL,B_TX_MASTERDDC|B_TX_MASTERHOST);
        HDMITX_WriteI2C_Byte(client, REG_TX_DDC_CMD,CMD_FIFO_CLR);

        for(TimeOut = 0 ; TimeOut < 200 ; TimeOut++)
        {
            ucdata = HDMITX_ReadI2C_Byte(client, REG_TX_DDC_STATUS);

            if(ucdata&B_TX_DDC_DONE)
            {
                break ;
            }
            if((ucdata & B_TX_DDC_ERROR)||(HDMITX_ReadI2C_Byte(client, REG_TX_INT_STAT1) & B_TX_INT_DDC_BUS_HANG))
            {
                HDMITX_DEBUG_PRINTF(("Called hdmitx_AboutDDC()\n"));
                hdmitx_AbortDDC(client);
                return ER_FAIL ;
        ...
        HDMITX_WriteI2C_Byte(client, REG_TX_DDC_MASTER_CTRL,B_TX_MASTERDDC|B_TX_MASTERHOST);
        HDMITX_WriteI2C_Byte(client, REG_TX_DDC_HEADER,DDC_EDID_ADDRESS); // for EDID ucdata get
        HDMITX_WriteI2C_Byte(client, REG_TX_DDC_REQOFF,bCurrOffset);
        HDMITX_WriteI2C_Byte(client, REG_TX_DDC_REQCOUNT,(BYTE)ReqCount);
        HDMITX_WriteI2C_Byte(client, REG_TX_DDC_EDIDSEG,bSegment);
        HDMITX_WriteI2C_Byte(client, REG_TX_DDC_CMD,CMD_EDID_READ);
        ...
        do
        {
            *(pBuff++) = HDMITX_ReadI2C_Byte(client, REG_TX_DDC_READFIFO);
            ReqCount -- ;
        }while(ReqCount > 0);
...
void hdmitx_ClearDDCFIFO(struct i2c_client *client)
{
    HDMITX_WriteI2C_Byte(client, REG_TX_DDC_MASTER_CTRL,B_TX_MASTERDDC|B_TX_MASTERHOST);
    HDMITX_WriteI2C_Byte(client, REG_TX_DDC_CMD,CMD_FIFO_CLR);
}

void hdmitx_GenerateDDCSCLK(struct i2c_client *client)
{
    HDMITX_WriteI2C_Byte(client, REG_TX_DDC_MASTER_CTRL,B_TX_MASTERDDC|B_TX_MASTERHOST);
    HDMITX_WriteI2C_Byte(client, REG_TX_DDC_CMD,CMD_GEN_SCLCLK);
}
...
void hdmitx_AbortDDC(struct i2c_client *client)
{
    BYTE CPDesire,SWReset,DDCMaster ;
    BYTE uc, timeout, i ;
    // save the SW reset,DDC master,and CP Desire setting.
    SWReset = HDMITX_ReadI2C_Byte(client, REG_TX_SW_RST);
    CPDesire = HDMITX_ReadI2C_Byte(client, REG_TX_HDCP_DESIRE);
    DDCMaster = HDMITX_ReadI2C_Byte(client, REG_TX_DDC_MASTER_CTRL);

    HDMITX_WriteI2C_Byte(client, REG_TX_HDCP_DESIRE,CPDesire&(~B_TX_CPDESIRE)); // @emily change order
    HDMITX_WriteI2C_Byte(client, REG_TX_SW_RST,SWReset|B_TX_HDCP_RST_HDMITX);         // @emily change order
    HDMITX_WriteI2C_Byte(client, REG_TX_DDC_MASTER_CTRL,B_TX_MASTERDDC|B_TX_MASTERHOST);
    ...
    for( i = 0 ; i < 2 ; i++ )
    {
        HDMITX_WriteI2C_Byte(client, REG_TX_DDC_CMD,CMD_DDC_ABORT);

        for( timeout = 0 ; timeout < 200 ; timeout++ )
        {
            uc = HDMITX_ReadI2C_Byte(client, REG_TX_DDC_STATUS);
            if (uc&B_TX_DDC_DONE)
            {
                break ; // success
            }
...
void WaitTxVidStable(struct i2c_client *client);
void hdmitx_SetInputMode(struct i2c_client *client, BYTE InputMode,BYTE bInputSignalType);
void hdmitx_SetCSCScale(struct i2c_client *client, BYTE bInputMode,BYTE bOutputMode);
void hdmitx_SetupAFE(struct i2c_client *client, VIDEOPCLKLEVEL PCLKLevel);
void hdmitx_FireAFE(struct i2c_client *client);
...
void HDMITX_DisableVideoOutput(struct i2c_client *client)
{
    BYTE uc = HDMITX_ReadI2C_Byte(client, REG_TX_SW_RST) | B_HDMITX_VID_RST ;
    HDMITX_WriteI2C_Byte(client, REG_TX_SW_RST,uc);
    HDMITX_WriteI2C_Byte(client, REG_TX_AFE_DRV_CTRL,B_TX_AFE_DRV_RST|B_TX_AFE_DRV_PWD);
    HDMITX_SetI2C_Byte(client, 0x62, 0x90, 0x00);
    HDMITX_SetI2C_Byte(client, 0x64, 0x89, 0x00);
}

BOOL HDMITX_EnableVideoOutput(struct i2c_client *client, VIDEOPCLKLEVEL level,BYTE inputColorMode,BYTE outputColorMode,BYTE bHDMI)
{
    // bInputVideoMode,bOutputVideoMode,cat66121_hdmi->hdmiTxDev.bInputVideoSignalType,bAudioInputType,should be configured by upper F/W or loaded from EEPROM.
    ...
    {
	    case PCLK_HIGH:
		    HDMITX_WriteI2C_Byte(client, REG_TX_PLL_CTRL,0x30 /*0xff*/);
		    break ;
	    default:
		    HDMITX_WriteI2C_Byte(client, REG_TX_PLL_CTRL,0x00);
            break ;
    }
    HDMITX_WriteI2C_Byte(client, REG_TX_SW_RST,B_HDMITX_VID_RST|B_HDMITX_AUD_RST|B_TX_AREF_RST|B_TX_HDCP_RST_HDMITX);

    cat66121_hdmi->hdmiTxDev.bHDMIMode = (BYTE)bHDMI ;
    // 2009/12/09 added by jau-chih.tseng@ite.com.tw
    Switch_HDMITX_Bank(client, 1);
    HDMITX_WriteI2C_Byte(client, REG_TX_AVIINFO_DB1,0x00);
    Switch_HDMITX_Bank(client, 0);
    //~jau-chih.tseng@ite.com.tw

    if(cat66121_hdmi->hdmiTxDev.bHDMIMode)
    {
        setHDMITX_AVMute(client, TRUE);
    }
    hdmitx_SetInputMode(client, inputColorMode,cat66121_hdmi->hdmiTxDev.bInputVideoSignalType);

    hdmitx_SetCSCScale(client, inputColorMode,outputColorMode);

    if(cat66121_hdmi->hdmiTxDev.bHDMIMode)
    {
        HDMITX_WriteI2C_Byte(client, REG_TX_HDMI_MODE,B_TX_HDMI_MODE);
    }
    else
    {
        HDMITX_WriteI2C_Byte(client, REG_TX_HDMI_MODE,B_TX_DVI_MODE);
    }
    ...
    hdmitx_SetupAFE(client, level); // pass if High Freq request
    HDMITX_WriteI2C_Byte(client, REG_TX_SW_RST,          B_HDMITX_AUD_RST|B_TX_AREF_RST|B_TX_HDCP_RST_HDMITX);
...
//////////////////////////////////////////////////////////////////////
BOOL setHDMITX_VideoSignalType(struct i2c_client *client, BYTE inputSignalType)
{
	struct cat66121_hdmi_pdata *cat66121_hdmi = i2c_get_clientdata(client);

	cat66121_hdmi->hdmiTxDev.bInputVideoSignalType = inputSignalType ;
    // hdmitx_SetInputMode(inputColorMode,cat66121_hdmi->hdmiTxDev.bInputVideoSignalType);
    return TRUE ;
}
...
void WaitTxVidStable(struct i2c_client *client)
{
    ...
    {
        delay1ms(15);
        if((HDMITX_ReadI2C_Byte(client, REG_TX_SYS_STATUS) & B_TXVIDSTABLE) == 0 )
        {
            continue ;
        }
        delay1ms(15);
...
void setHDMITX_ColorDepthPhase(struct i2c_client *client, BYTE ColorDepth,BYTE bPhase)
{
#ifdef IT6615
    BYTE uc ;
    BYTE bColorDepth ;
    ...
    */
    ...
    Switch_HDMITX_Bank(client, 0);
    HDMITX_SetI2C_Byte(client, REG_TX_GCP,B_TX_COLOR_DEPTH_MASK ,bColorDepth);
	HDMITX_DEBUG_PRINTF(("setHDMITX_ColorDepthPhase(%02X), regC1 = %02X\n",(int)bColorDepth,(int)HDMITX_ReadI2C_Byte(client, REG_TX_GCP))) ;
#endif
}
...
BOOL setHDMITX_SyncEmbeddedByVIC(struct i2c_client *client, BYTE VIC,BYTE bInputType)
{
    int i ;
    BYTE fmt_index=0;
    ...
    //if( bInputSignalType & T_MODE_SYNCEMB )
    {
        ...
        HDMITX_SetI2C_Byte(client, 0x90, 0x06, Pol);
        // write H2ndVRRise
        HDMITX_SetI2C_Byte(client, 0x90, 0xF0, (H2ndVRRise&0x0F)<<4);
        HDMITX_WriteI2C_Byte(client, 0x91, (H2ndVRRise&0x0FF0)>>4);
        // write HRS/HRE
        HDMITX_WriteI2C_Byte(client, 0x95, HRS&0xFF);
        HDMITX_WriteI2C_Byte(client, 0x96, HRE&0xFF);
        HDMITX_WriteI2C_Byte(client, 0x97, ((HRE&0x0F00)>>4)+((HRS&0x0F00)>>8));
        // write VRS/VRE
        HDMITX_WriteI2C_Byte(client, 0xa0, VRS&0xFF);
        HDMITX_WriteI2C_Byte(client, 0xa1, ((VRE&0x0F)<<4)+((VRS&0x0F00)>>8));
        HDMITX_WriteI2C_Byte(client, 0xa2, VRS2nd&0xFF);
        HDMITX_WriteI2C_Byte(client, 0xa6, (VRE2nd&0xF0)+((VRE&0xF0)>>4));
        HDMITX_WriteI2C_Byte(client, 0xa3, ((VRE2nd&0x0F)<<4)+((VRS2nd&0xF00)>>8));
        HDMITX_WriteI2C_Byte(client, 0xa4, H2ndVRRise&0xFF);
        HDMITX_WriteI2C_Byte(client, 0xa5, (/*EnDEOnly*/0<<5)+((TimingTable[fmt_index].Scan==INTERLACE)?(1<<4):0)+((H2ndVRRise&0xF00)>>8));
        HDMITX_SetI2C_Byte(client, 0xb1, 0x51, ((HRE&0x1000)>>6)+((HRS&0x1000)>>8)+((HDES&0x1000)>>12));
        HDMITX_SetI2C_Byte(client, 0xb2, 0x05, ((H2ndVRRise&0x1000)>>10)+((H2ndVRRise&0x1000)>>12));
    }
    return TRUE ;
}
...
void hdmitx_SetInputMode(struct i2c_client *client, BYTE InputColorMode,BYTE bInputSignalType)
{
    BYTE ucData ;

    ucData = HDMITX_ReadI2C_Byte(client, REG_TX_INPUT_MODE);
    ...
    HDMITX_WriteI2C_Byte(client, REG_TX_INPUT_MODE,ucData);
}
...
void hdmitx_SetCSCScale(struct i2c_client *client, BYTE bInputMode,BYTE bOutputMode)
{
    ...
    if(csc == B_HDMITX_CSC_RGB2YUV)
    {
        ...
        case F_VIDMODE_ITU709|F_VIDMODE_16_235:
            ...
            for( i = 0 ; i < SIZEOF_CSCMTX ; i++ ){ HDMITX_WriteI2C_Byte(client, REG_TX_CSC_YOFF+i,bCSCMtx_RGB2YUV_ITU709_16_235[i]) ; HDMITX_DEBUG_PRINTF(("reg%02X <- %02X\n",(int)(i+REG_TX_CSC_YOFF),(int)bCSCMtx_RGB2YUV_ITU709_16_235[i]));}
            break ;
        case F_VIDMODE_ITU709|F_VIDMODE_0_255:
            HDMITX_DEBUG_PRINTF(("ITU709 0-255 "));
            for( i = 0 ; i < SIZEOF_CSCMTX ; i++ ){ HDMITX_WriteI2C_Byte(client, REG_TX_CSC_YOFF+i,bCSCMtx_RGB2YUV_ITU709_0_255[i]) ; HDMITX_DEBUG_PRINTF(("reg%02X <- %02X\n",(int)(i+REG_TX_CSC_YOFF),(int)bCSCMtx_RGB2YUV_ITU709_0_255[i]));}
            break ;
        case F_VIDMODE_ITU601|F_VIDMODE_16_235:
            HDMITX_DEBUG_PRINTF(("ITU601 16-235 "));
            for( i = 0 ; i < SIZEOF_CSCMTX ; i++ ){ HDMITX_WriteI2C_Byte(client, REG_TX_CSC_YOFF+i,bCSCMtx_RGB2YUV_ITU601_16_235[i]) ; HDMITX_DEBUG_PRINTF(("reg%02X <- %02X\n",(int)(i+REG_TX_CSC_YOFF),(int)bCSCMtx_RGB2YUV_ITU601_16_235[i]));}
            break ;
        case F_VIDMODE_ITU601|F_VIDMODE_0_255:
        default:
            HDMITX_DEBUG_PRINTF(("ITU601 0-255 "));
            for( i = 0 ; i < SIZEOF_CSCMTX ; i++ ){ HDMITX_WriteI2C_Byte(client, REG_TX_CSC_YOFF+i,bCSCMtx_RGB2YUV_ITU601_0_255[i]) ; HDMITX_DEBUG_PRINTF(("reg%02X <- %02X\n",(int)(i+REG_TX_CSC_YOFF),(int)bCSCMtx_RGB2YUV_ITU601_0_255[i]));}
            break ;
        }
    }
    #endif
    ...
    if (csc == B_HDMITX_CSC_YUV2RGB)
    {
        ...
        case F_VIDMODE_ITU709|F_VIDMODE_16_235:
            HDMITX_DEBUG_PRINTF(("ITU709 16-235 "));
            for( i = 0 ; i < SIZEOF_CSCMTX ; i++ ){ HDMITX_WriteI2C_Byte(client, REG_TX_CSC_YOFF+i,bCSCMtx_YUV2RGB_ITU709_16_235[i]) ; HDMITX_DEBUG_PRINTF(("reg%02X <- %02X\n",(int)(i+REG_TX_CSC_YOFF),(int)bCSCMtx_YUV2RGB_ITU709_16_235[i]));}
            break ;
        case F_VIDMODE_ITU709|F_VIDMODE_0_255:
            HDMITX_DEBUG_PRINTF(("ITU709 0-255 "));
            for( i = 0 ; i < SIZEOF_CSCMTX ; i++ ){ HDMITX_WriteI2C_Byte(client, REG_TX_CSC_YOFF+i,bCSCMtx_YUV2RGB_ITU709_0_255[i]) ; HDMITX_DEBUG_PRINTF(("reg%02X <- %02X\n",(int)(i+REG_TX_CSC_YOFF),(int)bCSCMtx_YUV2RGB_ITU709_0_255[i]));}
            break ;
        case F_VIDMODE_ITU601|F_VIDMODE_16_235:
            HDMITX_DEBUG_PRINTF(("ITU601 16-235 "));
            for( i = 0 ; i < SIZEOF_CSCMTX ; i++ ){ HDMITX_WriteI2C_Byte(client, REG_TX_CSC_YOFF+i,bCSCMtx_YUV2RGB_ITU601_16_235[i]) ; HDMITX_DEBUG_PRINTF(("reg%02X <- %02X\n",(int)(i+REG_TX_CSC_YOFF),(int)bCSCMtx_YUV2RGB_ITU601_16_235[i]));}
            break ;
        case F_VIDMODE_ITU601|F_VIDMODE_0_255:
        default:
            HDMITX_DEBUG_PRINTF(("ITU601 0-255 "));
            for( i = 0 ; i < SIZEOF_CSCMTX ; i++ ){ HDMITX_WriteI2C_Byte(client, REG_TX_CSC_YOFF+i,bCSCMtx_YUV2RGB_ITU601_0_255[i]) ; HDMITX_DEBUG_PRINTF(("reg%02X <- %02X\n",(int)(i+REG_TX_CSC_YOFF),(int)bCSCMtx_YUV2RGB_ITU601_0_255[i]));}
            break ;
        }
    }
    #endif
	...
	{
		HDMITX_SetI2C_Byte(client, 0xF, 0x10, 0x10);
	}
	else
	{
		HDMITX_SetI2C_Byte(client, 0xF, 0x10, 0x00);
	}
    ucData = HDMITX_ReadI2C_Byte(client, REG_TX_CSC_CTRL) & ~(M_TX_CSC_SEL|B_TX_DNFREE_GO|B_TX_EN_DITHER|B_TX_EN_UDFILTER);
    ucData |= filter|csc ;

    HDMITX_WriteI2C_Byte(client, REG_TX_CSC_CTRL,ucData);
...
void hdmitx_SetupAFE(struct i2c_client *client, VIDEOPCLKLEVEL level)
{

    HDMITX_WriteI2C_Byte(client, REG_TX_AFE_DRV_CTRL,B_TX_AFE_DRV_RST);/* 0x10 */
    switch(level)
    {
        case PCLK_HIGH:
            HDMITX_SetI2C_Byte(client, 0x62, 0x90, 0x80);
            HDMITX_SetI2C_Byte(client, 0x64, 0x89, 0x80);
            HDMITX_SetI2C_Byte(client, 0x68, 0x10, 0x80);
            HDMITX_DEBUG_PRINTF(("hdmitx_SetupAFE()===================HIGHT\n"));
            break ;
        default:
            HDMITX_SetI2C_Byte(client, 0x62, 0x90, 0x10);
            HDMITX_SetI2C_Byte(client, 0x64, 0x89, 0x09);
            HDMITX_SetI2C_Byte(client, 0x68, 0x10, 0x10);
            HDMITX_DEBUG_PRINTF(("hdmitx_SetupAFE()===================LOW\n"));
            break ;
    }
    HDMITX_SetI2C_Byte(client, REG_TX_SW_RST,B_TX_REF_RST_HDMITX|B_HDMITX_VID_RST,0);
    HDMITX_WriteI2C_Byte(client, REG_TX_AFE_DRV_CTRL,0);
    delay1ms(1);
}
...
void setHDMITX_ChStat(struct i2c_client *client, BYTE ucIEC60958ChStat[])
{
    BYTE uc ;

    Switch_HDMITX_Bank(client, 1);
    uc = (ucIEC60958ChStat[0] <<1)& 0x7C ;
    HDMITX_WriteI2C_Byte(client, REG_TX_AUDCHST_MODE,uc);
    HDMITX_WriteI2C_Byte(client, REG_TX_AUDCHST_CAT,ucIEC60958ChStat[1]); // 192, audio CATEGORY
    HDMITX_WriteI2C_Byte(client, REG_TX_AUDCHST_SRCNUM,ucIEC60958ChStat[2]&0xF);
    HDMITX_WriteI2C_Byte(client, REG_TX_AUD0CHST_CHTNUM,(ucIEC60958ChStat[2]>>4)&0xF);
    HDMITX_WriteI2C_Byte(client, REG_TX_AUDCHST_CA_FS,ucIEC60958ChStat[3]); // choose clock
    HDMITX_WriteI2C_Byte(client, REG_TX_AUDCHST_OFS_WL,ucIEC60958ChStat[4]);
    Switch_HDMITX_Bank(client, 0);
}

void setHDMITX_UpdateChStatFs(struct i2c_client *client, ULONG Fs)
{
    BYTE uc ;
    ...
    Switch_HDMITX_Bank(client, 1);
    uc = HDMITX_ReadI2C_Byte(client, REG_TX_AUDCHST_CA_FS); // choose clock
    HDMITX_WriteI2C_Byte(client, REG_TX_AUDCHST_CA_FS,uc); // choose clock
    uc &= 0xF0 ;
    uc |= (Fs&0xF);

    uc = HDMITX_ReadI2C_Byte(client, REG_TX_AUDCHST_OFS_WL);
    uc &= 0xF ;
    uc |= ((~Fs) << 4)&0xF0 ;
    HDMITX_WriteI2C_Byte(client, REG_TX_AUDCHST_OFS_WL,uc);

    Switch_HDMITX_Bank(client, 0);
}

void setHDMITX_LPCMAudio(struct i2c_client *client, BYTE AudioSrcNum, BYTE AudSWL, BOOL bSPDIF)
{
    ...
    AudioEnable = 0 ;
    AudioFormat = cat66121_hdmi->hdmiTxDev.bOutputAudioMode ;
    ...
    cat66121_hdmi->hdmiTxDev.bAudioChannelEnable=AudioEnable;

    Switch_HDMITX_Bank(client, 0);
    HDMITX_WriteI2C_Byte(client, REG_TX_AUDIO_CTRL0,AudioEnable&0xF0);

    HDMITX_WriteI2C_Byte(client, REG_TX_AUDIO_CTRL1,AudioFormat); // regE1 bOutputAudioMode should be loaded from ROM image.
    HDMITX_WriteI2C_Byte(client, REG_TX_AUDIO_FIFOMAP,0xE4); // default mapping.
#ifdef USE_SPDIF_CHSTAT
    if( bSPDIF )
    {
        HDMITX_WriteI2C_Byte(client, REG_TX_AUDIO_CTRL3,B_TX_CHSTSEL);
    }
    else
    {
        HDMITX_WriteI2C_Byte(client, REG_TX_AUDIO_CTRL3,0);
    }
#else // not USE_SPDIF_CHSTAT
    HDMITX_WriteI2C_Byte(client, REG_TX_AUDIO_CTRL3,0);
#endif // USE_SPDIF_CHSTAT

    HDMITX_WriteI2C_Byte(client, REG_TX_AUD_SRCVALID_FLAT,0x00);
    HDMITX_WriteI2C_Byte(client, REG_TX_AUD_HDAUDIO,0x00); // regE5 = 0 ;

    if( bSPDIF )
    {
        BYTE i ;
        HDMITX_OrReg_Byte(client, 0x5c,(1<<6));
        for( i = 0 ; i < 100 ; i++ )
        {
            if(HDMITX_ReadI2C_Byte(client, REG_TX_CLK_STATUS2) & B_TX_OSF_LOCK)
            {
                break ; // stable clock.
            }
        }
...
void setHDMITX_NLPCMAudio(struct i2c_client *client, BOOL bSPDIF) // no Source Num, no I2S.
{
    BYTE AudioEnable, AudioFormat ;
    BYTE i ;
    ...
    HDMITX_WriteI2C_Byte(client, REG_TX_AUDIO_CTRL0, AudioEnable);
    //HDMITX_AndREG_Byte(client, REG_TX_SW_RST,~(B_HDMITX_AUD_RST|B_TX_AREF_RST));

    HDMITX_WriteI2C_Byte(client, REG_TX_AUDIO_CTRL1,0x01); // regE1 bOutputAudioMode should be loaded from ROM image.
    HDMITX_WriteI2C_Byte(client, REG_TX_AUDIO_FIFOMAP,0xE4); // default mapping.
    ...
    HDMITX_WriteI2C_Byte(client, REG_TX_AUDIO_CTRL0, AudioEnable|B_TX_AUD_EN_I2S0);
}

void setHDMITX_HBRAudio(struct i2c_client *client, BOOL bSPDIF)
{
	struct cat66121_hdmi_pdata *cat66121_hdmi = i2c_get_clientdata(client);
    ...
    // rst = HDMITX_ReadI2C_Byte(client, REG_TX_SW_RST);
	// rst &= ~(B_HDMITX_AUD_RST|B_TX_AREF_RST);

    // HDMITX_WriteI2C_Byte(client, REG_TX_SW_RST, rst | B_HDMITX_AUD_RST );

    HDMITX_WriteI2C_Byte(client, REG_TX_AUDIO_CTRL1,0x47); // regE1 bOutputAudioMode should be loaded from ROM image.
    HDMITX_WriteI2C_Byte(client, REG_TX_AUDIO_FIFOMAP,0xE4); // default mapping.
    ...
    {
        HDMITX_WriteI2C_Byte(client, REG_TX_AUDIO_CTRL0, M_TX_AUD_24BIT);
        HDMITX_WriteI2C_Byte(client, REG_TX_AUDIO_CTRL3,0);
    }
    HDMITX_WriteI2C_Byte(client, REG_TX_AUD_SRCVALID_FLAT,0x08);
    HDMITX_WriteI2C_Byte(client, REG_TX_AUD_HDAUDIO,B_TX_HBR); // regE5 = 0 ;
    ...
    if( bSPDIF )
    {
        ...
        HDMITX_WriteI2C_Byte(client, REG_TX_AUDIO_CTRL0, M_TX_AUD_24BIT|B_TX_AUD_SPDIF|B_TX_AUD_EN_SPDIF);
    }
    else
    {
        HDMITX_WriteI2C_Byte(client, REG_TX_AUDIO_CTRL0, M_TX_AUD_24BIT|B_TX_AUD_EN_I2S3|B_TX_AUD_EN_I2S2|B_TX_AUD_EN_I2S1|B_TX_AUD_EN_I2S0);
    }
    HDMITX_AndReg_Byte(client, 0x5c,~(1<<6));
    cat66121_hdmi->hdmiTxDev.bAudioChannelEnable=HDMITX_ReadI2C_Byte(client, REG_TX_AUDIO_CTRL0);
    // HDMITX_WriteI2C_Byte(client, REG_TX_SW_RST, rst  );
}

void setHDMITX_DSDAudio(struct i2c_client *client)
{
    // to be continue
    // BYTE rst;
    // rst = HDMITX_ReadI2C_Byte(client, REG_TX_SW_RST);

    //HDMITX_WriteI2C_Byte(client, REG_TX_SW_RST, rst | (B_HDMITX_AUD_RST|B_TX_AREF_RST) );

    HDMITX_WriteI2C_Byte(client, REG_TX_AUDIO_CTRL1,0x41); // regE1 bOutputAudioMode should be loaded from ROM image.
    HDMITX_WriteI2C_Byte(client, REG_TX_AUDIO_FIFOMAP,0xE4); // default mapping.
    ...
    HDMITX_WriteI2C_Byte(client, REG_TX_AUD_SRCVALID_FLAT,0x00);
    HDMITX_WriteI2C_Byte(client, REG_TX_AUD_HDAUDIO,B_TX_DSD); // regE5 = 0 ;
    //HDMITX_WriteI2C_Byte(client, REG_TX_SW_RST, rst & ~(B_HDMITX_AUD_RST|B_TX_AREF_RST) );
...
void HDMITX_DisableAudioOutput(struct i2c_client *client)
{
    //BYTE uc = (HDMITX_ReadI2C_Byte(client, REG_TX_SW_RST) | (B_HDMITX_AUD_RST | B_TX_AREF_RST));
    //HDMITX_WriteI2C_Byte(client, REG_TX_SW_RST,uc);
    AudioDelayCnt=AudioOutDelayCnt;
    LastRefaudfreqnum=0;
    HDMITX_SetI2C_Byte(client, REG_TX_SW_RST, (B_HDMITX_AUD_RST | B_TX_AREF_RST), (B_HDMITX_AUD_RST | B_TX_AREF_RST) );
    HDMITX_SetI2C_Byte(client, 0x0F, 0x10, 0x10 );
}

void HDMITX_EnableAudioOutput(struct i2c_client *client, BYTE AudioType, BOOL bSPDIF,  ULONG SampleFreq,  BYTE ChNum, BYTE *pIEC60958ChStat, ULONG TMDSClock)
{
    ...
    AudioDelayCnt=36;
    LastRefaudfreqnum=0;
    cat66121_hdmi->hdmiTxDev.TMDSClock=TMDSClock;
    cat66121_hdmi->hdmiTxDev.bAudioChannelEnable=0;
    cat66121_hdmi->hdmiTxDev.bSPDIF_OUT=bSPDIF;
    ...
    HDMITX_OrReg_Byte(client, REG_TX_SW_RST,(B_HDMITX_AUD_RST | B_TX_AREF_RST));
    HDMITX_WriteI2C_Byte(client, REG_TX_CLK_CTRL0,B_TX_AUTO_OVER_SAMPLING_CLOCK|B_TX_EXT_256FS|0x01);

    HDMITX_SetI2C_Byte(client, 0x0F, 0x10, 0x00 ); // power on the ACLK

    if(bSPDIF)
    {
        if(AudioType==T_AUDIO_HBR)
        {
            HDMITX_WriteI2C_Byte(client, REG_TX_CLK_CTRL0,0x81);
        }
        HDMITX_OrReg_Byte(client, REG_TX_AUDIO_CTRL0,B_TX_AUD_SPDIF);
    }
    else
    {
        HDMITX_AndReg_Byte(client, REG_TX_AUDIO_CTRL0,(~B_TX_AUD_SPDIF));
    }
    if( AudioType != T_AUDIO_DSD)
    {
        ...
        cat66121_hdmi->hdmiTxDev.bAudFs=AUDFS_OTHER;
    #else
        cat66121_hdmi->hdmiTxDev.bAudFs=Fs;
    #endif
        setHDMITX_NCTS(client, cat66121_hdmi->hdmiTxDev.bAudFs);
    ...
    HDMITX_SetI2C_Byte(client, REG_TX_SW_RST,(B_HDMITX_AUD_RST|B_TX_AREF_RST),B_TX_AREF_RST);
    ...
    case T_AUDIO_HBR:
        ...
        setHDMITX_ChStat(client, pIEC60958ChStat);
        setHDMITX_HBRAudio(client, bSPDIF);

        break ;
    case T_AUDIO_DSD:
        HDMITX_DEBUG_PRINTF(("T_AUDIO_DSD\n"));
        setHDMITX_DSDAudio(client);
        break ;
    case T_AUDIO_NLPCM:
        HDMITX_DEBUG_PRINTF(("T_AUDIO_NLPCM\n"));
        pIEC60958ChStat[0] |= 1<<1 ;
        setHDMITX_ChStat(client, pIEC60958ChStat);
        setHDMITX_NLPCMAudio(client, bSPDIF);
        break ;
    case T_AUDIO_LPCM:
        ...
        setHDMITX_ChStat(client, pIEC60958ChStat);
        setHDMITX_LPCMAudio(client, (ChNum+1)/2, SUPPORT_AUDI_AudSWL, bSPDIF);
        // can add auto adjust
        break ;
    }
    HDMITX_AndReg_Byte(client, REG_TX_INT_MASK1,(~B_TX_AUDIO_OVFLW_MASK));
    HDMITX_WriteI2C_Byte(client, REG_TX_AUDIO_CTRL0, cat66121_hdmi->hdmiTxDev.bAudioChannelEnable);

    HDMITX_SetI2C_Byte(client, REG_TX_SW_RST,(B_HDMITX_AUD_RST|B_TX_AREF_RST),0);
}

void hdmitx_AutoAdjustAudio(struct i2c_client *client)
{
    ...
    {
        Switch_HDMITX_Bank(client, 0);
        HDMITX_WriteI2C_Byte(client, 0xF8, 0xC3);
        HDMITX_WriteI2C_Byte(client, 0xF8, 0xA5);
        HDMITX_AndReg_Byte(client, REG_TX_PKT_SINGLE_CTRL,~B_TX_SW_CTS); // D[1] = 0, HW auto count CTS
        HDMITX_WriteI2C_Byte(client, 0xF8, 0xFF);
    }
    //delay1ms(50);
    Switch_HDMITX_Bank(client, 1);
    N = ((unsigned long)HDMITX_ReadI2C_Byte(client, REGPktAudN2)&0xF) << 16 ;
    N |= ((unsigned long)HDMITX_ReadI2C_Byte(client, REGPktAudN1)) <<8 ;
    N |= ((unsigned long)HDMITX_ReadI2C_Byte(client, REGPktAudN0));

    while(LoopCnt--)
    {   ULONG TempCTS=0;
        aCTS = ((unsigned long)HDMITX_ReadI2C_Byte(client, REGPktAudCTSCnt2)) << 12 ;
        aCTS |= ((unsigned long)HDMITX_ReadI2C_Byte(client, REGPktAudCTSCnt1)) <<4 ;
        aCTS |= ((unsigned long)HDMITX_ReadI2C_Byte(client, REGPktAudCTSCnt0)&0xf0)>>4  ;
        if(aCTS==TempCTS)
        {break;}
        TempCTS=aCTS;
    }
    ...
    uc = HDMITX_ReadI2C_Byte(client, REG_TX_GCP);

    cTMDSClock = cat66121_hdmi->hdmiTxDev.TMDSClock ;
    ...
    if(cat66121_hdmi->hdmiTxDev.bAudFs != fs)
    {
        cat66121_hdmi->hdmiTxDev.bAudFs=fs;
        setHDMITX_NCTS(client, cat66121_hdmi->hdmiTxDev.bAudFs); // set N, CTS by new generated clock.
        //CurrCTS=0;
        return;
    }
    return;
...
BOOL hdmitx_IsAudioChang(struct i2c_client *client)
{
    //ULONG pCTS=0;
    BYTE FreDiff=0,Refaudfreqnum;

    //Switch_HDMITX_Bank(client, 1);
    //pCTS = ((unsigned long)HDMITX_ReadI2C_Byte(client, REGPktAudCTSCnt2)) << 12 ;
    //pCTS |= ((unsigned long)HDMITX_ReadI2C_Byte(client, REGPktAudCTSCnt1)) <<4 ;
    //pCTS |= ((unsigned long)HDMITX_ReadI2C_Byte(client, REGPktAudCTSCnt0)&0xf0)>>4  ;
    //Switch_HDMITX_Bank(client, 0);
    Switch_HDMITX_Bank(client, 0);
    Refaudfreqnum=HDMITX_ReadI2C_Byte(client, 0x60);
    ...
    if((1<<4)&HDMITX_ReadI2C_Byte(client, 0x5f))
    {
        //printf("=======XXXXXXXXXXX=========\n");
        return FALSE;
    }
    ...
    if(3<FreDiff)
    {
        ...
        HDMITX_OrReg_Byte(client, REG_TX_PKT_SINGLE_CTRL,(1<<5));
        HDMITX_AndReg_Byte(client, REG_TX_AUDIO_CTRL0,0xF0);
        return TRUE;
    }
    else
    {
...
void setHDMITX_AudioChannelEnable(struct i2c_client *client, BOOL EnableAudio_b)
{
	struct cat66121_hdmi_pdata *cat66121_hdmi = i2c_get_clientdata(client);
    ...
    if(EnableAudio_b)
    {
        ...
        #ifdef SUPPORT_AUDIO_MONITOR
            if(hdmitx_IsAudioChang(client))
            {
                hdmitx_AutoAdjustAudio(client);
        #else
            if(AudioOutStatus==FALSE)
            {
                setHDMITX_NCTS(client, cat66121_hdmi->hdmiTxDev.bAudFs);
        #endif
                HDMITX_WriteI2C_Byte(client, REG_TX_AUD_SRCVALID_FLAT,0);
                HDMITX_OrReg_Byte(client, REG_TX_PKT_SINGLE_CTRL,(1<<5));
                HDMITX_WriteI2C_Byte(client, REG_TX_AUDIO_CTRL0, cat66121_hdmi->hdmiTxDev.bAudioChannelEnable);
                //HDMITX_OrREG_Byte(0x59,(1<<2));  //for test
                HDMITX_AndReg_Byte(client, REG_TX_PKT_SINGLE_CTRL,(~0x3C));
                HDMITX_AndReg_Byte(client, REG_TX_PKT_SINGLE_CTRL,(~(1<<5)));
        ...
        else
        {
            ...
            if(0==(HDMITX_ReadI2C_Byte(client, REG_TX_CLK_STATUS2)&0x10))
            {
                AudioDelayCnt--;
            }
            else
...
void setHDMITX_NCTS(struct i2c_client *client, BYTE Fs)
{
    ULONG n;
    ...
    if(B_TX_HBR & HDMITX_ReadI2C_Byte(client, REG_TX_AUD_HDAUDIO))
    {
        HBR_mode=TRUE;
    }
    else
    ...
    // tr_printf((" n = %ld\n",n));
    Switch_HDMITX_Bank(client, 1);
    HDMITX_WriteI2C_Byte(client, REGPktAudN0,(BYTE)((n)&0xFF));
    HDMITX_WriteI2C_Byte(client, REGPktAudN1,(BYTE)((n>>8)&0xFF));
    HDMITX_WriteI2C_Byte(client, REGPktAudN2,(BYTE)((n>>16)&0xF));
    ...
    if(bForceCTS)
    {
        ...
        {
            delay1ms(30);
            CTS = ((unsigned long)HDMITX_ReadI2C_Byte(client, REGPktAudCTSCnt2)) << 12 ;
            CTS |= ((unsigned long)HDMITX_ReadI2C_Byte(client, REGPktAudCTSCnt1)) <<4 ;
            CTS |= ((unsigned long)HDMITX_ReadI2C_Byte(client, REGPktAudCTSCnt0)&0xf0)>>4  ;
            if( CTS == 0)
            {
                continue;
            }
    ...
    HDMITX_WriteI2C_Byte(client, REGPktAudCTS0,(BYTE)((LastCTS)&0xFF));
    HDMITX_WriteI2C_Byte(client, REGPktAudCTS1,(BYTE)((LastCTS>>8)&0xFF));
    HDMITX_WriteI2C_Byte(client, REGPktAudCTS2,(BYTE)((LastCTS>>16)&0xF));
    Switch_HDMITX_Bank(client, 0);
#ifdef Force_CTS
    bForceCTS = TRUE;
#endif
    ...
    if(bForceCTS)
    {
        HDMITX_OrReg_Byte(client, REG_TX_PKT_SINGLE_CTRL,B_TX_SW_CTS); // D[1] = 0, HW auto count CTS
    }
    else
    {
        HDMITX_AndReg_Byte(client, REG_TX_PKT_SINGLE_CTRL,~B_TX_SW_CTS); // D[1] = 0, HW auto count CTS
    ...
    if(FALSE==HBR_mode) //LPCM
    {
        ...
        Switch_HDMITX_Bank(client, 1);
        Fs = AUDFS_768KHz ;
        HDMITX_WriteI2C_Byte(client, REG_TX_AUDCHST_CA_FS,0x00|Fs);
        Fs = ~Fs ; // OFS is the one's complement of FS
        uData = (0x0f&HDMITX_ReadI2C_Byte(client, REG_TX_AUDCHST_OFS_WL));
        HDMITX_WriteI2C_Byte(client, REG_TX_AUDCHST_OFS_WL,(Fs<<4)|uData);
        Switch_HDMITX_Bank(client, 0);
    }
}
...
BOOL HDMITX_EnableVSInfoFrame(struct i2c_client *client, BYTE bEnable,BYTE *pVSInfoFrame)
{
    if(!bEnable)
    {
        hdmitx_DISABLE_VSDB_PKT(client);
        return TRUE ;
    }
    if(hdmitx_SetVSIInfoFrame(client, (VendorSpecific_InfoFrame *)pVSInfoFrame) == ER_SUCCESS)
    {
        return TRUE ;
    }
    return FALSE ;
...
BOOL HDMITX_EnableAVIInfoFrame(struct i2c_client *client, BYTE bEnable,BYTE *pAVIInfoFrame)
{
    if(!bEnable)
    {
        hdmitx_DISABLE_AVI_INFOFRM_PKT(client);
        return TRUE ;
    }
    if(hdmitx_SetAVIInfoFrame(client, (AVI_InfoFrame *)pAVIInfoFrame) == ER_SUCCESS)
    {
        return TRUE ;
    }
    return FALSE ;
...
BOOL HDMITX_EnableAudioInfoFrame(struct i2c_client *client, BYTE bEnable,BYTE *pAudioInfoFrame)
{
    if(!bEnable)
    {
        hdmitx_DISABLE_AVI_INFOFRM_PKT(client);
    ...
    if(hdmitx_SetAudioInfoFrame(client, (Audio_InfoFrame *)pAudioInfoFrame) == ER_SUCCESS)
    {
        return TRUE ;
    }
    return FALSE ;
...
SYS_STATUS hdmitx_SetAVIInfoFrame(struct i2c_client *client, AVI_InfoFrame *pAVIInfoFrame)
{
    int i ;
    byte checksum ;
    ...
    Switch_HDMITX_Bank(client, 1);
    HDMITX_WriteI2C_Byte(client, REG_TX_AVIINFO_DB1,pAVIInfoFrame->pktbyte.AVI_DB[0]);
    HDMITX_WriteI2C_Byte(client, REG_TX_AVIINFO_DB2,pAVIInfoFrame->pktbyte.AVI_DB[1]);
    HDMITX_WriteI2C_Byte(client, REG_TX_AVIINFO_DB3,pAVIInfoFrame->pktbyte.AVI_DB[2]);
    HDMITX_WriteI2C_Byte(client, REG_TX_AVIINFO_DB4,pAVIInfoFrame->pktbyte.AVI_DB[3]);
    HDMITX_WriteI2C_Byte(client, REG_TX_AVIINFO_DB5,pAVIInfoFrame->pktbyte.AVI_DB[4]);
    HDMITX_WriteI2C_Byte(client, REG_TX_AVIINFO_DB6,pAVIInfoFrame->pktbyte.AVI_DB[5]);
    HDMITX_WriteI2C_Byte(client, REG_TX_AVIINFO_DB7,pAVIInfoFrame->pktbyte.AVI_DB[6]);
    HDMITX_WriteI2C_Byte(client, REG_TX_AVIINFO_DB8,pAVIInfoFrame->pktbyte.AVI_DB[7]);
    HDMITX_WriteI2C_Byte(client, REG_TX_AVIINFO_DB9,pAVIInfoFrame->pktbyte.AVI_DB[8]);
    HDMITX_WriteI2C_Byte(client, REG_TX_AVIINFO_DB10,pAVIInfoFrame->pktbyte.AVI_DB[9]);
    HDMITX_WriteI2C_Byte(client, REG_TX_AVIINFO_DB11,pAVIInfoFrame->pktbyte.AVI_DB[10]);
    HDMITX_WriteI2C_Byte(client, REG_TX_AVIINFO_DB12,pAVIInfoFrame->pktbyte.AVI_DB[11]);
    HDMITX_WriteI2C_Byte(client, REG_TX_AVIINFO_DB13,pAVIInfoFrame->pktbyte.AVI_DB[12]);
    ...
    HDMITX_DEBUG_PRINTF(("%02X ",(int)HDMITX_ReadI2C_Byte(client, REG_TX_AVIINFO_DB1)));
    HDMITX_DEBUG_PRINTF(("%02X ",(int)HDMITX_ReadI2C_Byte(client, REG_TX_AVIINFO_DB2)));
    HDMITX_DEBUG_PRINTF(("%02X ",(int)HDMITX_ReadI2C_Byte(client, REG_TX_AVIINFO_DB3)));
    HDMITX_DEBUG_PRINTF(("%02X ",(int)HDMITX_ReadI2C_Byte(client, REG_TX_AVIINFO_DB4)));
    HDMITX_DEBUG_PRINTF(("%02X ",(int)HDMITX_ReadI2C_Byte(client, REG_TX_AVIINFO_DB5)));
    HDMITX_DEBUG_PRINTF(("%02X ",(int)HDMITX_ReadI2C_Byte(client, REG_TX_AVIINFO_DB6)));
    HDMITX_DEBUG_PRINTF(("%02X ",(int)HDMITX_ReadI2C_Byte(client, REG_TX_AVIINFO_DB7)));
    HDMITX_DEBUG_PRINTF(("%02X ",(int)HDMITX_ReadI2C_Byte(client, REG_TX_AVIINFO_DB8)));
    HDMITX_DEBUG_PRINTF(("%02X ",(int)HDMITX_ReadI2C_Byte(client, REG_TX_AVIINFO_DB9)));
    HDMITX_DEBUG_PRINTF(("%02X ",(int)HDMITX_ReadI2C_Byte(client, REG_TX_AVIINFO_DB10)));
    HDMITX_DEBUG_PRINTF(("%02X ",(int)HDMITX_ReadI2C_Byte(client, REG_TX_AVIINFO_DB11)));
    HDMITX_DEBUG_PRINTF(("%02X ",(int)HDMITX_ReadI2C_Byte(client, REG_TX_AVIINFO_DB12)));
    HDMITX_DEBUG_PRINTF(("%02X ",(int)HDMITX_ReadI2C_Byte(client, REG_TX_AVIINFO_DB13)));
    HDMITX_DEBUG_PRINTF(("\n"));
    */
    checksum -= AVI_INFOFRAME_VER+AVI_INFOFRAME_TYPE+AVI_INFOFRAME_LEN ;
    HDMITX_WriteI2C_Byte(client, REG_TX_AVIINFO_SUM,checksum);

    Switch_HDMITX_Bank(client, 0);
    hdmitx_ENABLE_AVI_INFOFRM_PKT(client);
    return ER_SUCCESS ;
}
...
SYS_STATUS hdmitx_SetAudioInfoFrame(struct i2c_client *client, Audio_InfoFrame *pAudioInfoFrame)
{
    BYTE checksum ;
    ...
    HDMITX_WriteI2C_Byte(client, REG_TX_PKT_AUDINFO_CC,pAudioInfoFrame->pktbyte.AUD_DB[0]);
    checksum -= HDMITX_ReadI2C_Byte(client, REG_TX_PKT_AUDINFO_CC); checksum &= 0xFF ;
    HDMITX_WriteI2C_Byte(client, REG_TX_PKT_AUDINFO_SF,pAudioInfoFrame->pktbyte.AUD_DB[1]);
    checksum -= HDMITX_ReadI2C_Byte(client, REG_TX_PKT_AUDINFO_SF); checksum &= 0xFF ;
    HDMITX_WriteI2C_Byte(client, REG_TX_PKT_AUDINFO_CA,pAudioInfoFrame->pktbyte.AUD_DB[3]);
    checksum -= HDMITX_ReadI2C_Byte(client, REG_TX_PKT_AUDINFO_CA); checksum &= 0xFF ;
    HDMITX_WriteI2C_Byte(client, REG_TX_PKT_AUDINFO_DM_LSV,pAudioInfoFrame->pktbyte.AUD_DB[4]);
    checksum -= HDMITX_ReadI2C_Byte(client, REG_TX_PKT_AUDINFO_DM_LSV); checksum &= 0xFF ;

    HDMITX_WriteI2C_Byte(client, REG_TX_PKT_AUDINFO_SUM,checksum);

    Switch_HDMITX_Bank(client, 0);
    hdmitx_ENABLE_AUD_INFOFRM_PKT(client);
    return ER_SUCCESS ;
}
...
SYS_STATUS hdmitx_SetSPDInfoFrame(struct i2c_client *client, SPD_InfoFrame *pSPDInfoFrame)
{
    int i ;
    BYTE ucData ;
    ...
    for(i = 0,ucData = 0 ; i < 25 ; i++)
    {
        ...
        HDMITX_WriteI2C_Byte(client, REG_TX_PKT_SPDINFO_PB1+i,pSPDInfoFrame->pktbyte.SPD_DB[i]);
    }
    ucData -= SPD_INFOFRAME_VER+SPD_INFOFRAME_TYPE+SPD_INFOFRAME_LEN ;
    HDMITX_WriteI2C_Byte(client, REG_TX_PKT_SPDINFO_SUM,ucData); // checksum
    Switch_HDMITX_Bank(client, 0);
    hdmitx_ENABLE_SPD_INFOFRM_PKT(client);
    return ER_SUCCESS ;
}
...
SYS_STATUS hdmitx_SetMPEGInfoFrame(struct i2c_client *client, MPEG_InfoFrame *pMPGInfoFrame)
{
    int i ;
    BYTE ucData ;
    ...
    HDMITX_WriteI2C_Byte(client, REG_TX_PKT_MPGINFO_FMT,pMPGInfoFrame->info.FieldRepeat|(pMPGInfoFrame->info.MpegFrame<<1));
    HDMITX_WriteI2C_Byte(client, REG_TX_PKG_MPGINFO_DB0,pMPGInfoFrame->pktbyte.MPG_DB[0]);
    HDMITX_WriteI2C_Byte(client, REG_TX_PKG_MPGINFO_DB1,pMPGInfoFrame->pktbyte.MPG_DB[1]);
    HDMITX_WriteI2C_Byte(client, REG_TX_PKG_MPGINFO_DB2,pMPGInfoFrame->pktbyte.MPG_DB[2]);
    HDMITX_WriteI2C_Byte(client, REG_TX_PKG_MPGINFO_DB3,pMPGInfoFrame->pktbyte.MPG_DB[3]);
    ...
    HDMITX_WriteI2C_Byte(client, REG_TX_PKG_MPGINFO_SUM,ucData);
...
SYS_STATUS hdmitx_SetVSIInfoFrame(struct i2c_client *client, VendorSpecific_InfoFrame *pVSIInfoFrame)
{
    BYTE ucData=0 ;
    ...
    Switch_HDMITX_Bank(client, 1);
    HDMITX_WriteI2C_Byte(client, 0x80,pVSIInfoFrame->pktbyte.VS_DB[3]);
    HDMITX_WriteI2C_Byte(client, 0x81,pVSIInfoFrame->pktbyte.VS_DB[4]);
    ...
    if(  pVSIInfoFrame->pktbyte.VS_DB[4] & (1<<7 ))
    {
        ...
        HDMITX_WriteI2C_Byte(client, 0x82,pVSIInfoFrame->pktbyte.VS_DB[5]);
    ...
    HDMITX_WriteI2C_Byte(client, 0x83,pVSIInfoFrame->pktbyte.CheckSum);
    Switch_HDMITX_Bank(client, 0);
    HDMITX_WriteI2C_Byte(client, REG_TX_3D_INFO_CTRL,B_TX_ENABLE_PKT|B_TX_REPEAT_PKT);
    return ER_SUCCESS ;
}

SYS_STATUS hdmitx_Set_GeneralPurpose_PKT(struct i2c_client *client, BYTE *pData)
{
    int i ;
    ...
    for( i = 0x38 ; i <= 0x56 ; i++)
    {
        HDMITX_WriteI2C_Byte(client, i, pData[i-0x38] );
    }
    Switch_HDMITX_Bank(client, 0);
    hdmitx_ENABLE_GeneralPurpose_PKT(client);
...
#if Debug_message
void DumpHDMITXReg(struct i2c_client *client)
{
    int i,j ;
    BYTE ucData ;
    ...
    for(i = 0 ; i < 0x100 ; i+=16)
    {
        ...
        for(j = 0 ; j < 16 ; j++)
        {
            if( (i+j)!= 0x17)
            {
                ucData = HDMITX_ReadI2C_Byte(client, (BYTE)((i+j)&0xFF));
