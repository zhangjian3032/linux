...
BOOL HDMITX_EnableHDCP(struct i2c_client *client, BYTE bEnable)
{
    ...
    if(bEnable)
    {
        if(ER_FAIL == hdmitx_hdcp_Authenticate(client))
        {
            HDCP_DEBUG_PRINTF(("ER_FAIL == hdmitx_hdcp_Authenticate\n"));
            hdmitx_hdcp_ResetAuth(client);
			return FALSE ;
        }
...
//{
    ...
    //HDCP_DEBUG_PRINTF((" getHDMITX_AuthenticationDone() = %s\n",cat66121_hdmi->bAuthenticated?"TRUE":"FALSE" ));
//    return cat66121_hdmi->bAuthenticated;
//}

//////////////////////////////////////////////////////////////////////
// Authentication
//////////////////////////////////////////////////////////////////////
void hdmitx_hdcp_ClearAuthInterrupt(struct i2c_client *client)
{
    // BYTE uc ;
    // uc = HDMITX_ReadI2C_Byte(client, REG_TX_INT_MASK2) & (~(B_TX_KSVLISTCHK_MASK|B_TX_AUTH_DONE_MASK|B_TX_AUTH_FAIL_MASK));
    HDMITX_SetI2C_Byte(client, REG_TX_INT_MASK2, B_TX_KSVLISTCHK_MASK|B_TX_AUTH_DONE_MASK|B_TX_AUTH_FAIL_MASK, 0);
    HDMITX_WriteI2C_Byte(client, REG_TX_INT_CLR0,B_TX_CLR_AUTH_FAIL|B_TX_CLR_AUTH_DONE|B_TX_CLR_KSVLISTCHK);
    HDMITX_WriteI2C_Byte(client, REG_TX_INT_CLR1,0);
    HDMITX_WriteI2C_Byte(client, REG_TX_SYS_STATUS,B_TX_INTACTDONE);
}

void hdmitx_hdcp_ResetAuth(struct i2c_client *client)
{
    HDMITX_WriteI2C_Byte(client, REG_TX_LISTCTRL,0);
    HDMITX_WriteI2C_Byte(client, REG_TX_HDCP_DESIRE,0);
    HDMITX_OrReg_Byte(client, REG_TX_SW_RST,B_TX_HDCP_RST_HDMITX);
    HDMITX_WriteI2C_Byte(client, REG_TX_DDC_MASTER_CTRL,B_TX_MASTERDDC|B_TX_MASTERHOST);
    hdmitx_hdcp_ClearAuthInterrupt(client);
...
void hdmitx_hdcp_Auth_Fire(struct i2c_client *client)
{
    // HDCP_DEBUG_PRINTF(("hdmitx_hdcp_Auth_Fire():\n"));
    HDMITX_WriteI2C_Byte(client, REG_TX_DDC_MASTER_CTRL,B_TX_MASTERDDC|B_TX_MASTERHDCP); // MASTERHDCP,no need command but fire.
    HDMITX_WriteI2C_Byte(client, REG_TX_AUTHFIRE,1);
}
...
void hdmitx_hdcp_StartAnCipher(struct i2c_client *client)
{
    HDMITX_WriteI2C_Byte(client, REG_TX_AN_GENERATE,B_TX_START_CIPHER_GEN);
...
void hdmitx_hdcp_StopAnCipher(struct i2c_client *client)
{
    HDMITX_WriteI2C_Byte(client, REG_TX_AN_GENERATE,B_TX_STOP_CIPHER_GEN);
}
...
void hdmitx_hdcp_GenerateAn(struct i2c_client *client)
{
    BYTE Data[8];
    BYTE i=0;
#if 1
    hdmitx_hdcp_StartAnCipher(client);
    // HDMITX_WriteI2C_Byte(client, REG_TX_AN_GENERATE,B_TX_START_CIPHER_GEN);
    // delay1ms(1); // delay 1 ms
    // HDMITX_WriteI2C_Byte(client, REG_TX_AN_GENERATE,B_TX_STOP_CIPHER_GEN);

    hdmitx_hdcp_StopAnCipher(client);

    Switch_HDMITX_Bank(client, 0);
    // new An is ready in reg30
    HDMITX_ReadI2C_ByteN(client, REG_TX_AN_GEN,Data,8);
#else
    ...
    for(i=0;i<8;i++)
    {
        HDMITX_WriteI2C_Byte(client, REG_TX_AN+i,Data[i]);
    }
    //HDMITX_WriteI2C_ByteN(client, REG_TX_AN,Data,8);
}
...
SYS_STATUS hdmitx_hdcp_GetBCaps(struct i2c_client *client, PBYTE pBCaps ,PUSHORT pBStatus)
{
    BYTE ucdata ;
    BYTE TimeOut ;
    ...
    Switch_HDMITX_Bank(client, 0);
    HDMITX_WriteI2C_Byte(client, REG_TX_DDC_MASTER_CTRL,B_TX_MASTERDDC|B_TX_MASTERHOST);
    HDMITX_WriteI2C_Byte(client, REG_TX_DDC_HEADER,DDC_HDCP_ADDRESS);
    HDMITX_WriteI2C_Byte(client, REG_TX_DDC_REQOFF,0x40); // BCaps offset
    HDMITX_WriteI2C_Byte(client, REG_TX_DDC_REQCOUNT,3);
    HDMITX_WriteI2C_Byte(client, REG_TX_DDC_CMD,CMD_DDC_SEQ_BURSTREAD);
...
#if 1
    ucdata = HDMITX_ReadI2C_Byte(client, REG_TX_BSTAT+1);

    *pBStatus = (USHORT)ucdata ;
    *pBStatus <<= 8 ;
    ucdata = HDMITX_ReadI2C_Byte(client, REG_TX_BSTAT);
    *pBStatus |= ((USHORT)ucdata&0xFF);
    *pBCaps = HDMITX_ReadI2C_Byte(client, REG_TX_BCAP);
#else
...
SYS_STATUS hdmitx_hdcp_GetBKSV(struct i2c_client *client, BYTE *pBKSV)
{
    BYTE ucdata ;
    BYTE TimeOut ;
    ...
    HDMITX_WriteI2C_Byte(client, REG_TX_DDC_MASTER_CTRL,B_TX_MASTERDDC|B_TX_MASTERHOST);
    HDMITX_WriteI2C_Byte(client, REG_TX_DDC_HEADER,DDC_HDCP_ADDRESS);
    HDMITX_WriteI2C_Byte(client, REG_TX_DDC_REQOFF,0x00); // BKSV offset
    HDMITX_WriteI2C_Byte(client, REG_TX_DDC_REQCOUNT,5);
    HDMITX_WriteI2C_Byte(client, REG_TX_DDC_CMD,CMD_DDC_SEQ_BURSTREAD);
    ...
    HDMITX_ReadI2C_ByteN(client, REG_TX_BKSV,(PBYTE)pBKSV,5);
...
// Side-Effect:
//  1. cat66121_hdmi->bAuthenticated global variable will be TRUE when authenticated.
...
void hdmitx_hdcp_Reset(struct i2c_client *client)
{
    BYTE uc ;
    uc = HDMITX_ReadI2C_Byte(client, REG_TX_SW_RST) | B_TX_HDCP_RST_HDMITX ;
    HDMITX_WriteI2C_Byte(client, REG_TX_SW_RST,uc);
    HDMITX_WriteI2C_Byte(client, REG_TX_HDCP_DESIRE,0);
    HDMITX_WriteI2C_Byte(client, REG_TX_LISTCTRL,0);
    HDMITX_WriteI2C_Byte(client, REG_TX_DDC_MASTER_CTRL,B_TX_MASTERHOST);
    hdmitx_ClearDDCFIFO(client);
    hdmitx_AbortDDC(client);
}

SYS_STATUS hdmitx_hdcp_Authenticate(struct i2c_client *client)
{
    BYTE ucdata ;
    BYTE BCaps ;
    USHORT BStatus ;
 ...
 //   BYTE revoked = FALSE ;
    ...
    cat66121_hdmi->hdmiTxDev.bAuthenticated = FALSE ;
    if(0==(B_TXVIDSTABLE&HDMITX_ReadI2C_Byte(client, REG_TX_SYS_STATUS)))
    {
    ...
    for( TimeOut = 0 ; TimeOut < 80 ; TimeOut++ )
    {
        ...
        if(hdmitx_hdcp_GetBCaps(client, &BCaps,&BStatus) != ER_SUCCESS)
        {
        ...
        if(B_TX_HDMI_MODE == (HDMITX_ReadI2C_Byte(client, REG_TX_HDMI_MODE) & B_TX_HDMI_MODE ))
        {
    ...
    */
    ...
    hdmitx_hdcp_GetBKSV(client, BKSV);
    ...
    Switch_HDMITX_Bank(client, 0); // switch bank action should start on direct register writting of each function.

    HDMITX_AndReg_Byte(client, REG_TX_SW_RST,~(B_TX_HDCP_RST_HDMITX));

    HDMITX_WriteI2C_Byte(client, REG_TX_HDCP_DESIRE,B_TX_CPDESIRE);
    hdmitx_hdcp_ClearAuthInterrupt(client);

    hdmitx_hdcp_GenerateAn(client);
    HDMITX_WriteI2C_Byte(client, REG_TX_LISTCTRL,0);
    cat66121_hdmi->hdmiTxDev.bAuthenticated = FALSE ;
    ...
    {
        hdmitx_hdcp_Auth_Fire(client);
        // wait for status ;

        for(TimeOut = 250 ; TimeOut > 0 ; TimeOut --)
        {
            delay1ms(5); // delay 1ms
            ucdata = HDMITX_ReadI2C_Byte(client, REG_TX_AUTH_STAT);
            ...
            ucdata = HDMITX_ReadI2C_Byte(client, REG_TX_INT_STAT2);
            if(ucdata & B_TX_INT_AUTH_FAIL)
            {

                HDMITX_WriteI2C_Byte(client, REG_TX_INT_CLR0,B_TX_CLR_AUTH_FAIL);
                HDMITX_WriteI2C_Byte(client, REG_TX_INT_CLR1,0);
                HDMITX_WriteI2C_Byte(client, REG_TX_SYS_STATUS,B_TX_INTACTDONE);
                HDMITX_WriteI2C_Byte(client, REG_TX_SYS_STATUS,0);
    ...
    return hdmitx_hdcp_Authenticate_Repeater(client);
}
...
SYS_STATUS hdmitx_hdcp_VerifyIntegration(struct i2c_client *client)
{
    // if any interrupt issued a Auth fail,returned the Verify Integration fail.
    	struct cat66121_hdmi_pdata *cat66121_hdmi = i2c_get_clientdata(client);

    if(HDMITX_ReadI2C_Byte(client, REG_TX_INT_STAT1) & B_TX_INT_AUTH_FAIL)
    {
        hdmitx_hdcp_ClearAuthInterrupt(client);
        cat66121_hdmi->hdmiTxDev.bAuthenticated = FALSE ;
        return ER_FAIL ;
    }
    if(cat66121_hdmi->hdmiTxDev.bAuthenticated == TRUE)
    {
        return ER_SUCCESS ;
    }
    return ER_FAIL ;
...
void hdmitx_hdcp_CancelRepeaterAuthenticate(struct i2c_client *client)
{
    HDCP_DEBUG_PRINTF(("hdmitx_hdcp_CancelRepeaterAuthenticate"));
    HDMITX_WriteI2C_Byte(client, REG_TX_DDC_MASTER_CTRL,B_TX_MASTERDDC|B_TX_MASTERHOST);
    hdmitx_AbortDDC(client);
    HDMITX_WriteI2C_Byte(client, REG_TX_LISTCTRL,B_TX_LISTFAIL|B_TX_LISTDONE);
    hdmitx_hdcp_ClearAuthInterrupt(client);
}

void hdmitx_hdcp_ResumeRepeaterAuthenticate(struct i2c_client *client)
{
    HDMITX_WriteI2C_Byte(client, REG_TX_LISTCTRL,B_TX_LISTDONE);
    HDMITX_WriteI2C_Byte(client, REG_TX_DDC_MASTER_CTRL,B_TX_MASTERHDCP);
}
...
SYS_STATUS hdmitx_hdcp_GetKSVList(struct i2c_client *client, BYTE *pKSVList,BYTE cDownStream)
{
    BYTE TimeOut = 100 ;
    BYTE ucdata ;
    ...
    HDMITX_WriteI2C_Byte(client, REG_TX_DDC_MASTER_CTRL,B_TX_MASTERHOST);
    HDMITX_WriteI2C_Byte(client, REG_TX_DDC_HEADER,0x74);
    HDMITX_WriteI2C_Byte(client, REG_TX_DDC_REQOFF,0x43);
    HDMITX_WriteI2C_Byte(client, REG_TX_DDC_REQCOUNT,cDownStream * 5);
    HDMITX_WriteI2C_Byte(client, REG_TX_DDC_CMD,CMD_DDC_SEQ_BURSTREAD);
    ...
    {
        pKSVList[TimeOut] = HDMITX_ReadI2C_Byte(client, REG_TX_DDC_READFIFO);
...
SYS_STATUS hdmitx_hdcp_GetVr(struct i2c_client *client, BYTE *pVr)
{
    BYTE TimeOut  ;
    BYTE ucdata ;
    ...
    HDMITX_WriteI2C_Byte(client, REG_TX_DDC_MASTER_CTRL,B_TX_MASTERHOST);
    HDMITX_WriteI2C_Byte(client, REG_TX_DDC_HEADER,0x74);
    HDMITX_WriteI2C_Byte(client, REG_TX_DDC_REQOFF,0x20);
    HDMITX_WriteI2C_Byte(client, REG_TX_DDC_REQCOUNT,20);
    HDMITX_WriteI2C_Byte(client, REG_TX_DDC_CMD,CMD_DDC_SEQ_BURSTREAD);
    ...
    {
        HDMITX_WriteI2C_Byte(client, REG_TX_SHA_SEL ,TimeOut);
        pVr[TimeOut*4]  = (ULONG)HDMITX_ReadI2C_Byte(client, REG_TX_SHA_RD_BYTE1);
        pVr[TimeOut*4+1] = (ULONG)HDMITX_ReadI2C_Byte(client, REG_TX_SHA_RD_BYTE2);
        pVr[TimeOut*4+2] = (ULONG)HDMITX_ReadI2C_Byte(client, REG_TX_SHA_RD_BYTE3);
        pVr[TimeOut*4+3] = (ULONG)HDMITX_ReadI2C_Byte(client, REG_TX_SHA_RD_BYTE4);
//        HDCP_DEBUG_PRINTF(("V' = %02X %02X %02X %02X\n",(int)pVr[TimeOut*4],(int)pVr[TimeOut*4+1],(int)pVr[TimeOut*4+2],(int)pVr[TimeOut*4+3]));
    }
    return ER_SUCCESS ;
}

SYS_STATUS hdmitx_hdcp_GetM0(struct i2c_client *client, BYTE *pM0)
{
    int i ;
    ...
    HDMITX_WriteI2C_Byte(client, REG_TX_SHA_SEL,5); // read m0[31:0] from reg51~reg54
    pM0[0] = HDMITX_ReadI2C_Byte(client, REG_TX_SHA_RD_BYTE1);
    pM0[1] = HDMITX_ReadI2C_Byte(client, REG_TX_SHA_RD_BYTE2);
    pM0[2] = HDMITX_ReadI2C_Byte(client, REG_TX_SHA_RD_BYTE3);
    pM0[3] = HDMITX_ReadI2C_Byte(client, REG_TX_SHA_RD_BYTE4);
    HDMITX_WriteI2C_Byte(client, REG_TX_SHA_SEL,0); // read m0[39:32] from reg55
    pM0[4] = HDMITX_ReadI2C_Byte(client, REG_TX_AKSV_RD_BYTE5);
    HDMITX_WriteI2C_Byte(client, REG_TX_SHA_SEL,1); // read m0[47:40] from reg55
    pM0[5] = HDMITX_ReadI2C_Byte(client, REG_TX_AKSV_RD_BYTE5);
    HDMITX_WriteI2C_Byte(client, REG_TX_SHA_SEL,2); // read m0[55:48] from reg55
    pM0[6] = HDMITX_ReadI2C_Byte(client, REG_TX_AKSV_RD_BYTE5);
    HDMITX_WriteI2C_Byte(client, REG_TX_SHA_SEL,3); // read m0[63:56] from reg55
    pM0[7] = HDMITX_ReadI2C_Byte(client, REG_TX_AKSV_RD_BYTE5);
...
SYS_STATUS hdmitx_hdcp_Authenticate_Repeater(struct i2c_client *client)
{
    BYTE uc ,ii;
    // BYTE revoked ;
    // int i ;
    ...
    hdmitx_hdcp_GetBCaps(client, &BCaps,&BStatus);
    delay1ms(2);
    if((B_TX_INT_HPD_PLUG|B_TX_INT_RX_SENSE)&HDMITX_ReadI2C_Byte(client, REG_TX_INT_STAT1))
    {
    ...
    for(TimeOut = /*250*6*/10 ; TimeOut > 0 ; TimeOut --)
    {
        ...
        uc = HDMITX_ReadI2C_Byte(client, REG_TX_INT_STAT1);
        if(uc & B_TX_INT_DDC_BUS_HANG)
        {
            HDCP_DEBUG_PRINTF(("DDC Bus hang\n"));
            goto hdmitx_hdcp_Repeater_Fail ;
        }
        uc = HDMITX_ReadI2C_Byte(client, REG_TX_INT_STAT2);
        ...
        {
            HDMITX_WriteI2C_Byte(client, REG_TX_INT_CLR0,B_TX_CLR_KSVLISTCHK);
            HDMITX_WriteI2C_Byte(client, REG_TX_INT_CLR1,0);
            HDMITX_WriteI2C_Byte(client, REG_TX_SYS_STATUS,B_TX_INTACTDONE);
            HDMITX_WriteI2C_Byte(client, REG_TX_SYS_STATUS,0);
    ...
    for(TimeOut = 500 ; TimeOut > 0 ; TimeOut --)
    {
        ...
        if(hdmitx_hdcp_GetBCaps(client, &BCaps,&BStatus) == ER_FAIL)
        {
...
#ifdef SUPPORT_SHA
    if(hdmitx_hdcp_GetKSVList(client, KSVList,cDownStream) == ER_FAIL)
    {
        goto hdmitx_hdcp_Repeater_Fail ;
    }
#if 0
...
//            HDCP_DEBUG_PRINTF(("KSVFIFO[%d] = %02X %02X %02X %02X %02X is revoked\n",i,(int)KSVList[i*5],(int)KSVList[i*5+1],(int)KSVList[i*5+2],(int)KSVList[i*5+3],(int)KSVList[i*5+4]));
    ...
    if(hdmitx_hdcp_GetVr(client, Vr) == ER_FAIL)
    {
        goto hdmitx_hdcp_Repeater_Fail ;
    }
    if(hdmitx_hdcp_GetM0(client, M0) == ER_FAIL)
    {
        goto hdmitx_hdcp_Repeater_Fail ;
    }
    // do check SHA
    ...
    HDCP_DEBUG_PRINTF(("hdmitx_hdcp_Authenticate()-receiver: Authenticate SUCESS\n"));
    hdmitx_hdcp_ResumeRepeaterAuthenticate(client);
    cat66121_hdmi->hdmiTxDev.bAuthenticated = TRUE ;
    return ER_SUCCESS ;

hdmitx_hdcp_Repeater_Fail:
    hdmitx_hdcp_CancelRepeaterAuthenticate(client);
    return ER_FAIL ;
}
...
void hdmitx_hdcp_ResumeAuthentication(struct i2c_client *client)
{
    setHDMITX_AVMute(client, TRUE);
    if(hdmitx_hdcp_Authenticate(client) == ER_SUCCESS)
    {
    }
    setHDMITX_AVMute(client, FALSE);
}
