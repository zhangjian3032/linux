...
//Slave structure	
	...
	void				(*do_slave_xfer_done)(struct ast_i2c_bus *i2c_bus);	
	u8					slave_en;
...
static u32 select_i2c_clock(struct ast_i2c_bus *i2c_bus)
{
	...
	clk = ast_get_pclk();
//	printk("pclk = %d \n",clk);
	divider_ratio = clk / i2c_bus->ast_i2c_data->bus_clk;
...
//	printk("pclk = %d \n",clk);
	...
	for(i = 0; i < sizeof(i2c_timing_table)/sizeof(struct ast_i2c_timing_table); i++) {
		if((clk / i2c_timing_table[i].divisor) < i2c_bus->bus_clk) {
			break;
		}
	}
...
static void ast_slave_issue_alert(struct ast_i2c_bus *i2c_bus, u8 enable)
{
	...
	if(i2c_bus->bus_id > 3)
		...
		if(enable)
			ast_i2c_write(i2c_bus, ast_i2c_read(i2c_bus,I2C_CMD_REG) | AST_I2CD_S_ALT_EN, I2C_CMD_REG);
		else
			ast_i2c_write(i2c_bus, ast_i2c_read(i2c_bus,I2C_CMD_REG) & ~AST_I2CD_S_ALT_EN, I2C_CMD_REG);
	}
}

static void ast_slave_mode_enable(struct ast_i2c_bus *i2c_bus, struct i2c_msg *msgs)
{
	if(msgs->buf[0] == 1) {
		i2c_bus->slave_en = 1;
#ifdef AST_SOC_G5
		//for support general call 
		ast_i2c_write(i2c_bus, msgs->addr | AST_I2CD_SLAVE2_ENABLE, I2C_DEV_ADDR_REG);	
#else
...
static void ast_i2c_bus_init(struct ast_i2c_bus *i2c_bus)
{
...
#ifdef AST_SOC_G5
	ast_i2c_write(i2c_bus, AST_I2CD_BUS_AUTO_RELEASE | AST_I2CD_MASTER_EN, I2C_FUN_CTRL_REG);
#else
	...
	ret = of_property_read_u32(i2c_bus->dev->of_node,
				   "bus-frequency", &i2c_bus->bus_clk);
	if (ret < 0) {
		dev_err(i2c_bus->dev,
			"Could not read bus-frequency property\n");
		i2c_bus->bus_clk = 100000;
	}
...
//	ast_i2c_write(i2c_bus, 0x77743335, I2C_AC_TIMING_REG1);
	...
	//TODO
//	ast_i2c_write(i2c_bus, 0xAF, I2C_INTR_CTRL_REG);
	...
	switch(i2c_bus->slave_event) {
		case I2C_SLAVE_EVENT_START_WRITE:
			...
			if(i2c_bus->slave_msgs->addr) {
				...
				dev_err(i2c_bus->dev, "slave ERROR S[%d], sts[%x] 0x00[%x]~~ TODO ~~~~!!\n", i2c_bus->slave_operation, (ast_i2c_read(i2c_bus,I2C_CMD_REG) >> 19) & 0xf, ast_i2c_read(i2c_bus, I2C_FUN_CTRL_REG));
...
//			ast_i2c_write(i2c_bus, ast_i2c_read(i2c_bus, I2C_FUN_CTRL_REG) & ~(AST_I2CD_SLAVE_EN), I2C_FUN_CTRL_REG);	//will enable when slave rx stop	
			break;
...
ast_i2c_bus_error_recover(struct ast_i2c_bus *i2c_bus)
{
	...
	ast_i2c_write(i2c_bus, ast_i2c_read(i2c_bus, I2C_FUN_CTRL_REG) & ~(AST_I2CD_MASTER_EN | AST_I2CD_SLAVE_EN), I2C_FUN_CTRL_REG);	
	ast_i2c_write(i2c_bus, ast_i2c_read(i2c_bus, I2C_FUN_CTRL_REG) | (AST_I2CD_MASTER_EN | AST_I2CD_SLAVE_EN), I2C_FUN_CTRL_REG);		
...
static int ast_i2c_wait_bus_not_busy(struct ast_i2c_bus *i2c_bus)
{
	...
	while(ast_i2c_read(i2c_bus,I2C_CMD_REG) & AST_I2CD_BUS_BUSY_STS) {
		if(timeout<=0) {
			printk("%d-bus busy %x \n",i2c_bus->bus_id, ast_i2c_read(i2c_bus,I2C_CMD_REG));
...
//ast1070, ast1010 dma
static void ast_i2c_do_dec_dma_xfer(struct ast_i2c_bus *i2c_bus)	
{
	u32 cmd = 0;
	int i;

	i2c_bus->master_xfer_mode = DEC_DMA_XFER;
	i2c_bus->slave_xfer_mode = DEC_DMA_XFER;
	dev_dbg(i2c_bus->dev, "ast_i2c_do_dec_dma_xfer \n");
	if(i2c_bus->slave_operation == 1) { 
		if(i2c_bus->slave_msgs->flags & I2C_M_RD) {
			...
			dev_dbg(i2c_bus->dev, "(<--) slave tx DMA \n");
			...
			ast_i2c_write(i2c_bus, AST_I2C_DMA_SIZE-1, I2C_DMA_LEN_REG);
			ast_i2c_write(i2c_bus, AST_I2CD_TX_DMA_ENABLE | AST_I2CD_S_TX_CMD,I2C_CMD_REG);	
		} else {
			//DMA prepare rx
			dev_dbg(i2c_bus->dev, "(-->) slave rx DMA \n");
			ast_i2c_write(i2c_bus, i2c_bus->dma_addr, I2C_DMA_BASE_REG);
			ast_i2c_write(i2c_bus, (AST_I2C_DMA_SIZE-1), I2C_DMA_LEN_REG);
		...
		if(i2c_bus->master_xfer_cnt == -1) {
			...
			if(i2c_bus->master_msgs->flags & I2C_M_RD) {
				...
				ast_i2c_write(i2c_bus, (i2c_bus->master_msgs->addr <<1) |0x1, I2C_BYTE_BUF_REG);
//				tx_buf[0] = (i2c_bus->master_msgs->addr <<1); //+1
				...
				ast_i2c_write(i2c_bus, i2c_bus->dma_addr, I2C_DMA_BASE_REG);
				ast_i2c_write(i2c_bus, (i2c_bus->master_xfer_len-1), I2C_DMA_LEN_REG);
			}
			...
			if(i2c_bus->master_msgs->flags & I2C_M_RD) {
				...
				if((i2c_bus->master_msgs->len - i2c_bus->master_xfer_cnt) > AST_I2C_DMA_SIZE) {
					...
					if((i2c_bus->master_msgs->flags & I2C_M_RECV_LEN) && (i2c_bus->blk_r_flag == 0)) {
						...
						if((i2c_bus->master_xfer_len == 1) && (i2c_bus->xfer_last == 1)) {
							//change to byte mode
							cmd |= AST_I2CD_M_STOP_CMD | AST_I2CD_M_S_RX_CMD_LAST;
							cmd &= ~AST_I2CD_RX_DMA_ENABLE;
							i2c_bus->master_xfer_mode = BYTE_XFER;
							ast_i2c_write(i2c_bus, ast_i2c_read(i2c_bus,I2C_INTR_CTRL_REG) &
												~AST_I2CD_RX_DOWN_INTR_EN, I2C_INTR_CTRL_REG);
							
						} else if (i2c_bus->master_xfer_len > 1) {
							i2c_bus->master_xfer_len -=1;
							ast_i2c_write(i2c_bus, ast_i2c_read(i2c_bus,I2C_INTR_CTRL_REG) |
											AST_I2CD_RX_DOWN_INTR_EN, I2C_INTR_CTRL_REG);
						} else {
...
static void ast_i2c_do_inc_dma_xfer(struct ast_i2c_bus *i2c_bus)	
{
	...
	i2c_bus->master_xfer_mode = INC_DMA_XFER;
	i2c_bus->slave_xfer_mode = INC_DMA_XFER;
...
static void ast_i2c_do_pool_xfer(struct ast_i2c_bus *i2c_bus)	
{
	...
	ast_i2c_write(i2c_bus,
					...
					AST_I2CD_BUFF_SEL(i2c_bus->req_page->page_no),
					I2C_FUN_CTRL_REG);
#endif

	tx_buf = (u32 *) i2c_bus->req_page->page_addr;

#if defined(AST_SOC_G5)
	dev_dbg(i2c_bus->dev, "offset buffer = %x \n",i2c_bus->bus_id * 0x10);

	tx_buf = (u32*)(i2c_bus->req_page->page_addr + i2c_bus->bus_id * 0x10);
#endif


	if(i2c_bus->slave_operation) { 
		i2c_bus->slave_xfer_mode = BUFF_XFER;	
		if(i2c_bus->slave_msgs->flags & I2C_M_RD) {
			dev_dbg(i2c_bus->dev, "(<--) slave tx buf \n");
	
			if(i2c_bus->slave_msgs->len > i2c_bus->req_page->page_size)
				i2c_bus->slave_xfer_len = i2c_bus->req_page->page_size;
			else
				i2c_bus->slave_xfer_len = i2c_bus->slave_msgs->len;
			...
			for(i = 0; i< i2c_bus->slave_xfer_len; i++) {
				if(i%4 == 0)
					tx_buf[i/4] = 0;
				tx_buf[i/4] |= (i2c_bus->slave_msgs->buf[i2c_bus->slave_xfer_cnt + i] << ((i%4)*8)) ;
				dev_dbg(i2c_bus->dev, "[%x] ",tx_buf[i/4]);
			}
			ast_i2c_write(i2c_bus, AST_I2CD_TX_DATA_BUF_END_SET((i2c_bus->slave_xfer_len-1)) | 
						AST_I2CD_BUF_BASE_ADDR_SET((i2c_bus->req_page->page_addr_point)), 
						I2C_BUF_CTRL_REG);
		
			ast_i2c_write(i2c_bus, AST_I2CD_TX_BUFF_ENABLE | AST_I2CD_S_TX_CMD, I2C_CMD_REG);
		} else {
			//prepare for new rx
		...
		i2c_bus->master_xfer_mode = BUFF_XFER;	
		dev_dbg(i2c_bus->dev,"M cnt %d, xf len %d \n",i2c_bus->master_xfer_cnt, i2c_bus->master_msgs->len);
		if(i2c_bus->master_xfer_cnt == -1) {
			//send start 
			dev_dbg(i2c_bus->dev, " %sing %d byte%s %s 0x%02x\n",
...
//				tx_buf[0] = (i2c_bus->master_msgs->addr <<1); //+1
				...
				tx_buf[0] = (i2c_bus->master_msgs->addr <<1);	//+1
				//next data write
				if((i2c_bus->master_msgs->len + 1) > i2c_bus->req_page->page_size)
					i2c_bus->master_xfer_len = i2c_bus->req_page->page_size;
				else
					i2c_bus->master_xfer_len = i2c_bus->master_msgs->len + 1;
				...
				for(i = 1; i < i2c_bus->master_xfer_len; i++) {
					if(i%4 == 0)
						tx_buf[i/4] = 0;
					tx_buf[i/4] |= (i2c_bus->master_msgs->buf[i2c_bus->master_xfer_cnt + i] << ((i%4)*8)) ;
				}
				
				if ((i2c_bus->xfer_last == 1) && (i2c_bus->master_xfer_len == i2c_bus->master_msgs->len + 1)) {
					dev_dbg(i2c_bus->dev, "last stop \n");
					cmd |= AST_I2CD_M_STOP_CMD;
					ast_i2c_write(i2c_bus, ast_i2c_read(i2c_bus,I2C_INTR_CTRL_REG) &
										~AST_I2CD_TX_ACK_INTR_EN, I2C_INTR_CTRL_REG);				
				...
				ast_i2c_write(i2c_bus, 
							AST_I2CD_TX_DATA_BUF_END_SET((i2c_bus->master_xfer_len - 1)) |
							AST_I2CD_BUF_BASE_ADDR_SET(i2c_bus->req_page->page_addr_point),
							I2C_BUF_CTRL_REG);
			}
			ast_i2c_write(i2c_bus, cmd, I2C_CMD_REG);			
			...
			if(i2c_bus->master_msgs->flags & I2C_M_RD) {
				...
				if((i2c_bus->master_msgs->len - i2c_bus->master_xfer_cnt) > i2c_bus->req_page->page_size) {
					i2c_bus->master_xfer_len = i2c_bus->req_page->page_size;
					ast_i2c_write(i2c_bus, ast_i2c_read(i2c_bus,I2C_INTR_CTRL_REG) |
										AST_I2CD_RX_DOWN_INTR_EN, I2C_INTR_CTRL_REG);
				} else {
				...
				ast_i2c_write(i2c_bus,
									AST_I2CD_RX_BUF_END_ADDR_SET((i2c_bus->master_xfer_len-1))|
									AST_I2CD_BUF_BASE_ADDR_SET((i2c_bus->req_page->page_addr_point)),
									I2C_BUF_CTRL_REG);
				ast_i2c_write(i2c_bus, cmd, I2C_CMD_REG);
				dev_dbg(i2c_bus->dev, "rxfer size %d , cmd = %x \n",i2c_bus->master_xfer_len, cmd);
...
static void ast_i2c_do_byte_xfer(struct ast_i2c_bus *i2c_bus)
{
	...
	if(i2c_bus->slave_operation == 1) { 
		i2c_bus->slave_xfer_mode = BYTE_XFER;	
		...
		if(i2c_bus->slave_msgs->flags & I2C_M_RD) {
			...
			ast_i2c_write(i2c_bus, i2c_bus->slave_msgs->buf[i2c_bus->slave_xfer_cnt], I2C_BYTE_BUF_REG);
	...
	//				disable rx_dwn isr
				...
				ast_i2c_write(i2c_bus, *(xfer_buf + i2c_bus->master_xfer_cnt), I2C_BYTE_BUF_REG);
...
static void ast_i2c_slave_xfer_done(struct ast_i2c_bus *i2c_bus)
{
	...
	if (i2c_bus->slave_msgs->flags & I2C_M_RD) {
		...
		if(i2c_bus->slave_xfer_mode == BYTE_XFER) {
			xfer_len = 1;
		} else if (i2c_bus->slave_xfer_mode == BUFF_XFER) {
			xfer_len = AST_I2CD_TX_DATA_BUF_GET(ast_i2c_read(i2c_bus, I2C_BUF_CTRL_REG));
			xfer_len++;
			dev_dbg(i2c_bus->dev,"S tx buff done len %d \n",xfer_len);
		} else if (i2c_bus->slave_xfer_mode == DEC_DMA_XFER) {
			//DMA mode
			xfer_len = ast_i2c_read(i2c_bus, I2C_DMA_LEN_REG);
			if(xfer_len == 0)
				xfer_len = i2c_bus->slave_xfer_len;
			else
				xfer_len = i2c_bus->slave_xfer_len - xfer_len - 1;
			dev_dbg(i2c_bus->dev,"S tx tx dma done len %d \n",xfer_len);
		} else if (i2c_bus->slave_xfer_mode == INC_DMA_XFER) {
			//DMA mode
			xfer_len = ast_i2c_read(i2c_bus, I2C_DMA_LEN_REG);
...
//		dev_dbg(i2c_bus->dev, "rx \n");
		if(i2c_bus->slave_xfer_mode == BYTE_XFER) {
			i2c_bus->slave_msgs->buf[i2c_bus->slave_msgs->len] = ast_i2c_read(i2c_bus,I2C_BYTE_BUF_REG) >> 8;
...
#endif
			rx_buf = (u8 *)i2c_bus->req_page->page_addr;
#if defined(AST_SOC_G5)
			dev_dbg(i2c_bus->dev, "rx xfer_len %d, offset buffer = %x \n",xfer_len, i2c_bus->bus_id * 0x10);
			rx_buf = (u8 *) (i2c_bus->req_page->page_addr + i2c_bus->bus_id * 0x10);
#endif
			...
			for(i = 0; i < xfer_len ;i++) {
				...
				dev_dbg(i2c_bus->dev,"%d, [%x] \n",i2c_bus->slave_xfer_cnt+i ,i2c_bus->slave_msgs->buf[i2c_bus->slave_msgs->len+i]);
			}			
			...
			if ((ast_i2c_read(i2c_bus,I2C_INTR_STS_REG) & AST_I2CD_INTR_STS_NORMAL_STOP)) {
				free_pool_buff_page(i2c_bus->req_page);	 
			} else {
				ast_i2c_write(i2c_bus, 
							AST_I2CD_RX_BUF_END_ADDR_SET((AST_I2C_PAGE_SIZE - 1)) |
							AST_I2CD_BUF_BASE_ADDR_SET((i2c_bus->req_page->page_addr_point)),
			...
			for(i=0;i<xfer_len;i++) {
				i2c_bus->slave_msgs->buf[i2c_bus->slave_xfer_cnt+i] = i2c_bus->dma_buf[i];
				dev_dbg(i2c_bus->dev,"%d, [%x] \n",i2c_bus->slave_xfer_cnt+i ,i2c_bus->slave_msgs->buf[i2c_bus->slave_xfer_cnt+i]);
			}
		} else if (i2c_bus->slave_xfer_mode == INC_DMA_XFER) {
			//RX DMA DOWN
			xfer_len = ast_i2c_read(i2c_bus, I2C_DMA_LEN_REG);
			...
			for(i=0;i<xfer_len;i++) {
				...
				dev_dbg(i2c_bus->dev,"%d, [%02x] \n", i2c_bus->slave_msgs->len+i , i2c_bus->slave_msgs->buf[i2c_bus->slave_msgs->len+i]);
			}
			i2c_bus->slave_msgs->len += xfer_len;
		} else {
...
static void ast_i2c_master_xfer_done(struct ast_i2c_bus *i2c_bus)
{
	...
	dev_dbg(i2c_bus->dev, "ast_i2c_master_xfer_done mode[%d] %s\n",i2c_bus->master_xfer_mode, i2c_bus->master_msgs->flags & I2C_M_RD ? "read" : "write");	
	...
	if (i2c_bus->master_msgs->flags & I2C_M_RD) {
		...
		if(i2c_bus->master_xfer_mode == BYTE_XFER) {
			if ((i2c_bus->master_msgs->flags & I2C_M_RECV_LEN) && (i2c_bus->blk_r_flag == 0)) {
				i2c_bus->master_msgs->len += (ast_i2c_read(i2c_bus,I2C_BYTE_BUF_REG) & AST_I2CD_RX_BYTE_BUFFER) >> 8; 
				i2c_bus->blk_r_flag = 1;
				dev_dbg(i2c_bus->dev, "I2C_M_RECV_LEN %d \n", i2c_bus->master_msgs->len -1);			
			}
			xfer_len = 1;
			i2c_bus->master_msgs->buf[i2c_bus->master_xfer_cnt] = (ast_i2c_read(i2c_bus,I2C_BYTE_BUF_REG) & AST_I2CD_RX_BYTE_BUFFER) >> 8;
		} else if (i2c_bus->master_xfer_mode == BUFF_XFER) {
			pool_buf = (u8 *)i2c_bus->req_page->page_addr;
#if defined(AST_SOC_G5)
			dev_dbg(i2c_bus->dev, "offset buffer = %x \n",i2c_bus->bus_id * 0x10);

			pool_buf = (u8 *) (i2c_bus->req_page->page_addr + i2c_bus->bus_id * 0x10);
#endif
			...
			for(i = 0; i< xfer_len; i++) {
				...
				dev_dbg(i2c_bus->dev, "rx %d buff[%x]\n",i2c_bus->master_xfer_cnt+i, i2c_bus->master_msgs->buf[i2c_bus->master_xfer_cnt+i]);
			}
		...
		} else if (i2c_bus->master_xfer_mode == DEC_DMA_XFER) {
			//DMA Mode
			xfer_len = ast_i2c_read(i2c_bus, I2C_DMA_LEN_REG);
			if(xfer_len == 0)
				xfer_len = i2c_bus->master_xfer_len;
			else
				xfer_len = i2c_bus->master_xfer_len - xfer_len - 1;
			for(i = 0; i < xfer_len; i++) {
		...
		} else if (i2c_bus->master_xfer_mode == INC_DMA_XFER) {
			//DMA Mode
			xfer_len = ast_i2c_read(i2c_bus, I2C_DMA_LEN_REG);
			if(xfer_len == 0)
				xfer_len = i2c_bus->master_xfer_len;
...
static void ast_i2c_slave_addr_match(struct ast_i2c_bus *i2c_bus)
{
	...
	//request
	if(i2c_bus->slave_dma == BYTE_MODE) {
		i2c_bus->do_slave_xfer = ast_i2c_do_byte_xfer;
	} else if (i2c_bus->slave_dma == DEC_DMA_MODE) {
		i2c_bus->do_slave_xfer = ast_i2c_do_dec_dma_xfer;	
	} else if (i2c_bus->slave_dma == INC_DMA_MODE) {
		i2c_bus->do_slave_xfer = ast_i2c_do_inc_dma_xfer;
	} else {
		i2c_bus->do_slave_xfer = ast_i2c_do_byte_xfer;
	}
	i2c_bus->do_slave_xfer_done = ast_i2c_slave_xfer_done;
...
//	dev_dbg(i2c_bus->dev,"sts machine %x, slave_op %d \n", xfer_sts,i2c_bus->slave_operation);
	if(i2c_bus->slave_operation && i2c_bus->master_operation) {
		printk("%d-M-S error !!!! isr : %x, sts : %x ~~~~~~~~~~~~~~~~~~~\n ", i2c_bus->bus_id, sts, i2c_bus->state);
	}
	...
	if(i2c_bus->master_operation) {
		...
		if((sts & (AST_I2CD_INTR_STS_RX_DOWN | AST_I2CD_INTR_STS_SLAVE_MATCH)) == (AST_I2CD_INTR_STS_RX_DOWN | AST_I2CD_INTR_STS_SLAVE_MATCH)) {
			...
			//return slave nak 
			ast_i2c_write(i2c_bus, AST_I2CD_INTR_STS_RX_DOWN | AST_I2CD_INTR_STS_SLAVE_MATCH, I2C_INTR_STS_REG);			
		...
		switch(sts) {
			...
			case AST_I2CD_INTR_STS_TX_ACK | AST_I2CD_INTR_STS_NORMAL_STOP:
				if(i2c_bus->xfer_last) {
					...
					ast_i2c_write(i2c_bus, AST_I2CD_INTR_STS_TX_ACK | AST_I2CD_INTR_STS_NORMAL_STOP, I2C_INTR_STS_REG);
					//take care
					ast_i2c_write(i2c_bus, ast_i2c_read(i2c_bus,I2C_INTR_CTRL_REG) |
										AST_I2CD_TX_ACK_INTR_EN, I2C_INTR_CTRL_REG);
					ast_i2c_master_xfer_done(i2c_bus);
			...
			case AST_I2CD_INTR_STS_TX_NAK | AST_I2CD_INTR_STS_NORMAL_STOP:
				...
				ast_i2c_write(i2c_bus, AST_I2CD_INTR_STS_TX_NAK | AST_I2CD_INTR_STS_NORMAL_STOP, I2C_INTR_STS_REG);
			...
			case (AST_I2CD_INTR_STS_RX_DOWN | AST_I2CD_INTR_STS_NORMAL_STOP):
				if(i2c_bus->xfer_last) {	
					...
					ast_i2c_write(i2c_bus, AST_I2CD_INTR_STS_RX_DOWN | AST_I2CD_INTR_STS_NORMAL_STOP, I2C_INTR_STS_REG);
					//take care
					ast_i2c_write(i2c_bus, ast_i2c_read(i2c_bus,I2C_INTR_CTRL_REG) |
										AST_I2CD_RX_DOWN_INTR_EN, I2C_INTR_CTRL_REG);
					ast_i2c_master_xfer_done(i2c_bus);
				...
				break;
			default:
				printk("%d-Master No one care : isr :%x, state %x, ctrl : %x, M[%d], S[%d] \n", i2c_bus->bus_id, sts, i2c_bus->state, ast_i2c_read(i2c_bus,I2C_FUN_CTRL_REG), i2c_bus->master_operation, i2c_bus->slave_operation);
				ast_i2c_write(i2c_bus, ast_i2c_read(i2c_bus, I2C_FUN_CTRL_REG) & ~(AST_I2CD_MASTER_EN | AST_I2CD_SLAVE_EN), I2C_FUN_CTRL_REG);
				ast_i2c_write(i2c_bus, ast_i2c_read(i2c_bus, I2C_FUN_CTRL_REG) | AST_I2CD_MASTER_EN, I2C_FUN_CTRL_REG);
				complete(&i2c_bus->cmd_complete);				
				break;
		}		
	} else {
		...
		switch(sts) {
			...
			case AST_I2CD_INTR_STS_TX_NAK | AST_I2CD_INTR_STS_SLAVE_MATCH:
				...
				ast_i2c_slave_xfer_done(i2c_bus);
				ast_i2c_slave_addr_match(i2c_bus);
				ast_i2c_write(i2c_bus, AST_I2CD_INTR_STS_TX_NAK | AST_I2CD_INTR_STS_SLAVE_MATCH , I2C_INTR_STS_REG);
				break;
			case AST_I2CD_INTR_STS_RX_DOWN:
...
static int ast_i2c_do_msgs_xfer(struct ast_i2c_bus *i2c_bus, struct i2c_msg *msgs, int num)
{
	int i;
	int ret = 0;	
	long timeout = 0;
	...
	//request
	if(i2c_bus->master_dma == BYTE_MODE)
		i2c_bus->do_master_xfer = ast_i2c_do_byte_xfer;
	else if (i2c_bus->master_dma == DEC_DMA_MODE)
		i2c_bus->do_master_xfer = ast_i2c_do_dec_dma_xfer;	
	else if (i2c_bus->master_dma == INC_DMA_MODE)
...
//			dev_err(i2c_bus->dev, "0x00 [%x] M [%x], S [%x] \n",ast_i2c_read(i2c_bus,I2C_FUN_CTRL_REG) , i2c_bus->master_operation, i2c_bus->slave_operation);
		...
		if(i2c_bus->cmd_err != 0) {
//			printk("%d-cmd_err %d \n",i2c_bus->bus_id, i2c_bus->cmd_err);
...
stop:
	...
	if (timeout == 0) {
		...
		i2c_bus->state = (ast_i2c_read(i2c_bus,I2C_CMD_REG) >> 19) & 0xf;
		dev_dbg(i2c_bus->dev, "sts [%x], isr sts [%x] \n",i2c_bus->state, ast_i2c_read(i2c_bus,I2C_INTR_STS_REG));
		ret = -ETIMEDOUT;
	}

out:
	//Free ..
	if(i2c_bus->master_xfer_mode == BUFF_XFER) {
		free_pool_buff_page(i2c_bus->req_page);
	} 
	dev_dbg(i2c_bus->dev, "end xfer ret = %d, xfer mode[%d]\n",ret, i2c_bus->master_xfer_mode);
	return ret;
...
//	printk("%d-dwn 0x14[%x] M[%d] S[%d]\n", i2c_bus->bus_id, ast_i2c_read(i2c_bus,I2C_CMD_REG), i2c_bus->master_operation, i2c_bus->slave_operation);
	return ret;
}
...
static int ast_i2c_probe(struct platform_device *pdev)
{
	...
	} else if ((i2c_bus->master_dma >= DEC_DMA_MODE) || (i2c_bus->slave_dma >= DEC_DMA_MODE)) {
...
//	i2c_bus->adap.retries = 3;	
	 ...
	 */
	i2c_bus->adap.nr = pdev->id != -1 ? pdev->id : 0;
	...
	ret = devm_request_irq(&pdev->dev, i2c_bus->irq, i2c_ast_handler,
