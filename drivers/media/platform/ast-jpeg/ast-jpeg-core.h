...
/**
 * struct ast_jpeg - JPEG IP abstraction
...
//	struct video_device	*vfd_decoder;
	...
	struct device		*dev;
	struct ast_jpeg_variant *variant;
	u32			irq_status;
};

struct ast_jpeg_variant {
...
/**
 * ast_jpeg_marker - collection of markers from jpeg header
...
/**
 * ast_jpeg_q_data - parameters of one queue
 ...
 */
struct ast_jpeg_q_data {
	struct ast_jpeg_fmt	*fmt;
	u32			w;
	u32			h;
	u32			sos;
	struct ast_jpeg_marker	dht;
	struct ast_jpeg_marker	dqt;
	u32			sof;
	u32			sof_len;
	u32			components;
	u32			size;
...
/**
 * ast_jpeg_ctx - the device context data
...
struct ast_jpeg_ctx {
	...
	struct ast_jpeg_q_data	out_q;
	struct ast_jpeg_q_data	cap_q;
...
/**
 * ast_jpeg_buffer - description of memory containing input JPEG data
 * @size:	buffer size
 * @curr:	current position in the buffer
 * @data:	pointer to the data
 */
struct ast_jpeg_buffer {
	unsigned long size;
	unsigned long curr;
	unsigned long data;
};
...
/**
 * struct ast_jpeg_addr - JPEG converter physical address set for DMA
 * @y:   luminance plane physical address
 * @cb:  Cb plane physical address
 * @cr:  Cr plane physical address
 */
struct ast_jpeg_addr {
	u32     y;
	u32     cb;
	u32     cr;
};
