		...
		{	
			...
			if (chip_drv->probe(bank,ctrl_drv,spi_info) == 0)
			{
				...
				if (req == gotindex)
				{
					...
					private->flags = spi_info->flags;
#ifdef CONFIG_SFDP
					private->sfdp_data = spi_info->sfdp_data;
					private->read_opcode = spi_info->read_opcode;
#endif
					return 0;
				}
				gotindex++;
...
spi_flash_probe(struct map_info *map)
{
	...
	{
		printk("%d %d %d %d\n", (int)spi_info.regions[j].offset, spi_info.regions[j].erasesize, spi_info.regions[j].numblocks, spi_info.regions[j].numblocks);
