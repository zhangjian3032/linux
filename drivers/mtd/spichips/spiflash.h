...
#define SPI_FBYTE_ADDR_MODE		1
#define SPI_FLAG_STS_READ		2
#define SPI_FBYTE_SPANSION      (4 | SPI_FBYTE_ADDR_MODE)


#define FAST_READ_EN			1
//#define DUAL_SPI				1
#define BKUP_FAST_READ_EN		1
//#define BKUP_DUAL_SPI			1
...
struct spi_flash_info 
{
	...
	const __u16 dev_id;
	const u_long size;
	const u_long flags; // To identify SPI parts that supports 4Byte addressing
	const int numeraseregions;
	const struct mtd_erase_region_info regions[MAX_ERASE_REGIONS];
#ifdef CONFIG_SFDP
	const struct SFDP_DATA sfdp_data;
	const unsigned int read_opcode;
#endif
};
...
#ifdef CONFIG_SFDP
	int (*spi_transfer) (int bank, struct spi_info_cmd *spi_cmd_info,
			unsigned char *data, unsigned long datalen, int fbyteaddr);
	int (*spi_burst_read) (int bank, struct spi_info_cmd *spi_cmd_info,
            unsigned char *data, unsigned long datalen, int fbyteaddr);
#else
...
int spi_generic_enable_4byte_mode(int bank,struct spi_ctrl_driver *ctrl_drv);
int spi_generic_disable_4byte_mode(int bank,struct spi_ctrl_driver *ctrl_drv);
