/*
 * ast_pwm_fan.c - PWM Fan Tach driver for the Aspeed SoC
 *
...
ast_pwm_tacho_probe(struct platform_device *pdev)
{
	...
	ast_pwm_tacho = devm_kzalloc(&pdev->dev, sizeof(struct ast_pwm_tacho_data), GFP_KERNEL);
	if (!ast_pwm_tacho) {
		ret = -ENOMEM;
		goto out;
	}
	...
	ast_pwm_tacho->reg_base = devm_ioremap_resource(&pdev->dev, res);
...
static const struct of_device_id of_ast_pwm_tacho_match_table[] = {
	{ .compatible = "aspeed,ast-pwm-tacho", },
	{},
};
MODULE_DEVICE_TABLE(of, of_ast_pwm_tacho_match_table);
...
static struct platform_driver ast_pwm_tacho_driver = {
	...
	.driver		= {
		.name	= KBUILD_MODNAME,
		.of_match_table = of_ast_pwm_tacho_match_table,
	},
};

module_platform_driver(ast_pwm_tacho_driver);
