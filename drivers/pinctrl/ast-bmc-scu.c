...
ast_get_ahbclk(void)
{
	...
	BMC_SCUDBG("HPLL=%d, AXI_Div=%d, AHB_DIV = %d, AHB CLK=%d\n", hpll, axi_div, ahb_div, (hpll/axi_div)/ahb_div);	
...
ast_get_pclk(void)
{
        ...
        if((div >> 2) == 1) {
                BMC_SCUDBG("div=%d , return 24000000\n", div);
                return 24000000;
        } else {
                BMC_SCUDBG("hpll=%d, Div=%d, PCLK=%d\n", hpll, div, hpll/div);
                div = (div+1) << 1;
                return (hpll/div);
        }
...
ast_get_ahbclk(void)
{
	...
	BMC_SCUDBG("HPLL=%d, Div=%d, AHB CLK=%d\n", hpll, div, hpll/div);	
	return (hpll/div);
}
...
//	return 50000000;	
	...
	if(SCU_LHCLK_SOURCE_EN & clk_sel) {
		...
		div = (div+1) << 1;
#endif
		BMC_SCUDBG("HPLL=%d, Div=%d, LHCLK = %d\n", hpll, div, hpll/div);	
		return (hpll/div);
	} else {
		BMC_SCUMSG("LPC CLK not enable \n");
		return 0;
	}
