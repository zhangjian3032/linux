...
int tulip_rx(struct net_device *dev)
{
  ...
  while ( ! (tp->rx_ring[entry].status & cpu_to_le32(DescOwned))) 
...
#if 0 //DJ
      /* Change for Pilot to ignore collision errors in full duplex mode  */
      if ( ((status & 0xc8c2) == 0x8040) && (tp->full_duplex)   ) //DJ022310
	status &= ~0x8040;  //DJ022310
#endif   
#ifndef final_version 
      if (tulip_debug > 5)
	printk(KERN_DEBUG "%s: In tulip_rx(), entry %d %8.8x.\n",
	...
	break;
      if ((status & 0x38008300) != 0x0300) 
	{
	  //we expect MTU to be 1500 bytes , so we post buffers of 1536 bytes, so we expect
	  //both First and Last Desc to be set, if any of it is not set this is unexpected, 
	  //This can happen if we receive a Jumbo packet ,in such cases we ignore it.
	  if ((status & 0x38000300) != 0x0300) 
	    {
		pci_unmap_single(tp->pdev, tp->rx_buffers[entry].mapping,PKT_BUF_SZ, PCI_DMA_FROMDEVICE);
		dev_kfree_skb_irq(tp->rx_buffers[entry].skb);
		tp->rx_buffers[entry].skb = NULL;
		tp->rx_ring[entry].status &= ~cpu_to_le32(DescOwned);
		tp->rx_buffers[entry].mapping = 0;
		tp->stats.rx_length_errors++;
      ...
      else 
	{
	  ...
	  if (pkt_len < tulip_rx_copybreak
	      && (skb = dev_alloc_skb(pkt_len + 2)) != NULL) 
	    {
	      skb->dev = dev;
	      skb_reserve(skb, 2);	/* 16 byte align the IP header */
	      pci_dma_sync_single_for_cpu(tp->pdev,tp->rx_buffers[entry].mapping,pkt_len, PCI_DMA_FROMDEVICE);
#if ! defined(__alpha__)
#if (LINUX_VERSION_CODE >=  KERNEL_VERSION(2,6,26))
		
	      skb_copy_to_linear_data( skb, tp->rx_buffers[entry].skb->data,pkt_len);
#else
	      eth_copy_and_sum(skb, tp->rx_buffers[entry].skb->data,pkt_len, 0);
#endif
	      skb_put(skb, pkt_len);
#else
	      memcpy(skb_put(skb, pkt_len),tp->rx_buffers[entry].skb->data,pkt_len);
#endif
	      pci_dma_sync_single_for_device(tp->pdev,
					     tp->rx_buffers[entry].mapping,
					     pkt_len, PCI_DMA_FROMDEVICE);
...
#ifndef final_version
	      char *temp = skb_put(skb = tp->rx_buffers[entry].skb,pkt_len);
#else
	      skb_put(skb = tp->rx_buffers[entry].skb,pkt_len);
#endif
#ifndef final_version
	      if (tp->rx_buffers[entry].mapping !=le32_to_cpu(tp->rx_ring[entry].buffer1)) 
		{
		  printk(KERN_ERR "%s: Internal fault: The skbuff addresses "
	  ...
	  skb->protocol = eth_type_trans(skb, dev);
          //skb->ip_summed = CHECKSUM_COMPLETE;//Ashok
...
static inline unsigned int phy_interrupt (struct net_device *dev)
{
  ...
  volatile unsigned int lnk_sts_chg = 0;
  unsigned int smsc_lnk_sts=0;
  volatile unsigned char link_up = 0;
  static unsigned int smsc_spd_sts=0x6;
  volatile int link_chk_iterations = 2;

  spin_lock_irqsave(&tp->lock,flags);

  if (PHY_REALTEK == tp->phy_manuf_model_num)
  {
      int_status = tulip_mdio_read(dev, tp->phys[0], 0x13);
      if(int_status & 0x400)
      {
          lnk_sts_chg=1;

          // Link status has changed. Check if it is up/down
          link_up = (tulip_mdio_read(dev, tp->phys[0], 0x11) & 0x400) >> 10;
      }
  }
  else if ( (PHY_SMSC == tp->phy_manuf_model_num)||(PHY_SMSC_REVB == tp->phy_manuf_model_num))
  {
      smsc_lnk_sts = ( ( (tulip_mdio_read(dev, tp->phys[0], 31))>>2) & 0x7 );

      // TODO: See if there is a cleaner way to detect link status change in smsc
      if(smsc_lnk_sts!=smsc_spd_sts)
      {
          smsc_spd_sts =smsc_lnk_sts;
          lnk_sts_chg=1;

          link_up = (tulip_mdio_read(dev, tp->phys[0], 1) & 0x4) >> 2;
      }
  }
  else if (PHY_MICREL == tp->phy_manuf_model_num)
  {
	  int_status = tulip_mdio_read(dev, tp->phys[0],Mircel_Intr_Sts);
	  if((int_status & 0x28) == 0x28)
	  {
		  if ( (int_status & 0x4) != 0x4 ) 
		  {
			  lnk_sts_chg = 1;
			  link_up = (tulip_mdio_read(dev, tp->phys[0], 1) & 0x4) >> 2;
			  while( (link_up != 0x1) && (link_chk_iterations >= 0) )
			  {
				  link_chk_iterations--;
				  link_up = (tulip_mdio_read(dev, tp->phys[0], 1) & 0x4) >> 2;
				  udelay(1000);
			  ...
			  if (tulip_debug > 4)
				  printk("Micrel: link is down\n");
		  }
	  }
	  else if(int_status & 0x1)
	  {
		  lnk_sts_chg=1;
		  link_up = 1;
	  }
	  else if (int_status & 0x4)
	  {
		  lnk_sts_chg=1;
		  // Link down
		  link_up = 0;
  ...
  {
	int_status  = tulip_mdio_read(dev, tp->phys[0], 19);

	if ( ((int_status & (1<<12)) == (1<<12)) || ((int_status & (1<<10)) == (1<<10)) )
	{
		lnk_sts_chg=1;
		link_up = (tulip_mdio_read(dev, tp->phys[0], 1) & 0x4) >> 2;
		while( (link_up != 0x1) && (link_chk_iterations >= 0) )
...
#ifndef final_version
          if (tulip_debug > 4)
              printk("PHY interrupt status = 0x%x\n", int_status);
#endif
          tulip_stop_rxtx(tp);
          pilot_up(dev);
		  tp->csr6 |= 1<<21;
...
irqreturn_t tulip_interrupt(int irq, void *dev_instance)
{
  ...
  if ((csr5 & (NormalIntr|AbnormalIntr|TimerInt)) == 0)
    {
      return IRQ_HANDLED;
    }
    if ( (tp->flags & HAS_PHY_IRQ)&& ((csr5 & TimerInt)) )
    {
      iowrite32(csr5 & TimerInt, ioaddr + CSR5);
  ...
  do {
	...
	u32 tdes1 = le32_to_cpu(tp->tx_ring[entry].length);
	  ...
	  if (tp->tx_buffers[entry].skb == NULL) {
	    ...
	    if (tp->tx_buffers[entry].mapping)
	      pci_unmap_single(tp->pdev,tp->tx_buffers[entry].mapping,sizeof(tp->setup_frame),PCI_DMA_TODEVICE);
	    continue;
	  }
	  
	  /* Changed for Pilot to ignore carrier errors in full duplex mode - Amrut! */
	  if (!(status & 0x8000) || (status & 0x8c00 && tp->full_duplex == 1)) {  //DJ
#ifdef CONFIG_PILOT_SG
      ...
      if (tp->cur_tx - dirty_tx > TX_RING_SIZE) 
	{
      ...
      //DJ this is from open source  looks redundant to me
...
//PILOT_TX_INTERRUPT_COALISING
      if(tp->p_tx_coalising_en)
	{
	  //remove tx  interrupts
...
//DJ covered above  tp->stats.rx_fifo_errors += missed & 0x10000000 ? 0xfff : ((missed>>17)&0xfff);
...
#if 0
//Re-load the timer to reduce timer interrupts,we would be needing timer interrupts only we there is no RX interrupts
//most likely scenario is removal and re-insertion of cable.
...
void dma_sw_sleep(unsigned int delay)
{
  unsigned int i=0;
  for(i=0;i<delay;i++);
}
