 ...
 * This file is based on drivers/net/ethernet/dec/tulip/interrupt.c 
...
int tulip_refill_rx(struct net_device *dev)
{
	...
	for (; tp->cur_rx - tp->dirty_rx > 0; tp->dirty_rx++) {
		...
		if (tp->rx_buffers[entry].skb == NULL) {
			...
			mapping = dma_map_single(tp->ddev, skb->data, PKT_BUF_SZ,
					PCI_DMA_FROMDEVICE);
                               ...
                               if (pkt_len < tulip_rx_copybreak
                                   && (skb = dev_alloc_skb(pkt_len + 2)) != NULL) {
                                      ...
                                      dma_map_single(tp->ddev,
                                       ...
                                       dma_unmap_single(tp->ddev, tp->rx_buffers[entry].mapping,
...
int tulip_rx(struct net_device *dev)
{
  ...
  while ( ! (tp->rx_ring[entry].status & cpu_to_le32(DescOwned))) 
      ...
      if ((status & 0x38008300) != 0x0300) 
	{
	  ...
	  if ((status & 0x38000300) != 0x0300) 
	    {
		dma_unmap_single(tp->ddev, tp->rx_buffers[entry].mapping,PKT_BUF_SZ, PCI_DMA_FROMDEVICE);
	...
	u32 tdes1 = le32_to_cpu(tp->tx_ring[entry].length);
	  ...
	  if (tp->tx_buffers[entry].skb == NULL) {
	    ...
	    if (tp->tx_buffers[entry].mapping)
	      dma_unmap_single(tp->ddev,tp->tx_buffers[entry].mapping,sizeof(tp->setup_frame),PCI_DMA_TODEVICE);
	    continue;
	  }
