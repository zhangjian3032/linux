...
#include <linux/module.h>
#include <soc/aspeed/aspeed-espi.h>
...
aspeed_espi_pcp_rx(struct aspeed_espi_peripheral *espi_peripheral)
{
	...
	/* message cycle */
	if ((ESPI_GET_CYCLE_TYPE(rx_ctrl) & 0x10) == 0x10) {
		espi_peripheral->p_rx_channel.buf_len = 5;
		if (ESPI_GET_CYCLE_TYPE(rx_ctrl) & 0x1)
...
static int aspeed_espi_peripheral_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR_OR_NULL(espi_peripheral->map)) {
	...
	if(espi_peripheral->dma_mode) {
		espi_peripheral->p_rx_channel.buff = dma_alloc_coherent(dev,
									MAX_XFER_BUFF_SIZE * 3,
									&espi_peripheral->p_rx_channel.dma_addr,
									GFP_KERNEL);
		if (IS_ERR_OR_NULL(espi_peripheral->p_rx_channel.buff)) {
			dev_err(dev, "Failed to allocate RX DMA buffer\n");
			return -ENOMEM;
		}
		...
		regmap_update_bits(espi_peripheral->map, ASPEED_ESPI_CTRL,
				   ESPI_CTRL_PCNP_TX_DMA | ESPI_CTRL_PCP_RX_DMA | ESPI_CTRL_PCP_TX_DMA,
				   ESPI_CTRL_PCNP_TX_DMA | ESPI_CTRL_PCP_RX_DMA | ESPI_CTRL_PCP_TX_DMA);
	} else {
	...
	rc = devm_request_irq(&pdev->dev, espi_peripheral->irq, aspeed_espi_peripheral_irq,
			      IRQF_SHARED, dev_name(&pdev->dev), espi_peripheral);
	if (rc) {
		printk("espi peripheral Unable to get IRQ \n");
		return rc;
	}

	espi_peripheral->rst_irq = platform_get_irq(pdev, 1);
	if (espi_peripheral->rst_irq < 0) {
		dev_err(&pdev->dev, "no irq specified\n");
		return espi_peripheral->irq;
	}

	rc = devm_request_irq(&pdev->dev, espi_peripheral->rst_irq, aspeed_espi_peripheral_reset_irq, IRQF_SHARED,
	...
	pr_info("aspeed-espi-peripheral: module loaded\n");
	return 0;
}
