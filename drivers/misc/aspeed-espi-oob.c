		...
		struct bin_attribute *attr, char *buf, loff_t off, size_t count)
{
	...
	if((espi_oob->espi_version == 6) && (espi_oob->dma_mode == 1)) {
		...
		if(espi_oob->oob_rx_cmd[write_pt].cmd & BIT(31)) {
			...
			aspeed_espi_oob_write(espi_oob, BIT(31) | (write_pt << 16) | write_pt, ASPEED_ESPI_OOB_RX_WRITE_PT);
		} else {
			printk("rx empty \n");
			return 0;			
		}
		...
		if(espi_oob->oob_rx_buff) {
			...
			aspeed_espi_oob_write(espi_oob, ESPI_TRIGGER_PACKAGE, ASPEED_ESPI_OOB_RX_CTRL);
		} else {
			count = 0;
		}
	}
		...
		struct bin_attribute *attr, char *buf, loff_t off, size_t count)
{
	...
	if((espi_oob->espi_version == 6) && (espi_oob->dma_mode == 1)) {
		...
		if(((espi_oob->oob_tx_idx + 1) % OOB_TX_BUF_NUM) == aspeed_espi_oob_read(espi_oob, ASPEED_ESPI_OOB_TX_READ_PT)) {
			...
			aspeed_espi_oob_write(espi_oob, espi_oob->oob_tx_idx | BIT(31), ASPEED_ESPI_OOB_TX_WRITE_PT);
		}
	} else {
		if(aspeed_espi_oob_read(espi_oob, ASPEED_ESPI_OOB_TX_CTRL) & ESPI_TRIGGER_PACKAGE)
			...
			if(espi_oob->dma_mode) {
				...
				aspeed_espi_oob_write(espi_oob, espi_oob->oob_tx_buff_dma, ASPEED_ESPI_OOB_TX_DMA);				
			} else {
				for (i = 0; i < count; i++)
					aspeed_espi_oob_write(espi_oob, buf[i] , ASPEED_ESPI_OOB_TX_DATA);
			}
			aspeed_espi_oob_write(espi_oob, ESPI_TRIGGER_PACKAGE |  (count << 12) | (tag << 8) | ESPI_OOB_MESSAGE, ASPEED_ESPI_OOB_TX_CTRL);
		}
	}
	return count;
}
...
aspeed_espi_oob_rx(struct aspeed_espi_oob *espi_oob)
{
	...
	aspeed_espi_oob_write(espi_oob, ESPI_ISR_OOB_RX_COMP, ASPEED_ESPI_ISR);
...
static irqreturn_t aspeed_espi_oob_irq(int irq, void *arg)
{
	...
	if (sts & (ESPI_ISR_OOB_TX_ERR | ESPI_ISR_OOB_TX_ABORT | ESPI_ISR_OOB_RX_ABORT | ESPI_ISR_OOB_TX_COMP | ESPI_ISR_OOB_RX_COMP)) {
		if (sts & ESPI_ISR_OOB_TX_ERR) {
			...
			aspeed_espi_oob_write(espi_oob, ESPI_ISR_OOB_TX_ERR, ASPEED_ESPI_ISR);
		}

		if (sts & ESPI_ISR_OOB_TX_ABORT) {
			printk("ESPI_ISR_OOB_TX_ABORT \n");
			aspeed_espi_oob_write(espi_oob, ESPI_ISR_OOB_TX_ABORT, ASPEED_ESPI_ISR);
		}

		if (sts & ESPI_ISR_OOB_RX_ABORT) {
			printk("ESPI_ISR_OOB_RX_ABORT");
			aspeed_espi_oob_write(espi_oob, ESPI_ISR_OOB_RX_ABORT, ASPEED_ESPI_ISR);
		}

		if (sts & ESPI_ISR_OOB_TX_COMP) {
			printk("ESPI_ISR_OOB_TX_COMP \n");
			aspeed_espi_oob_write(espi_oob, ESPI_ISR_OOB_TX_COMP, ASPEED_ESPI_ISR);
		}
...
//controller init 
	if(espi_oob->dma_mode) {
		aspeed_espi_oob_write(espi_oob, espi_oob->oob_rx_cmd_dma, ASPEED_ESPI_OOB_RX_DMA);
		aspeed_espi_oob_write(espi_oob, OOB_RXCMD_DESC_NUM, ASPEED_ESPI_OOB_RX_RING_SIZE);
		aspeed_espi_oob_write(espi_oob, BIT(31), ASPEED_ESPI_OOB_RX_WRITE_PT);

		aspeed_espi_oob_write(espi_oob, espi_oob->oob_tx_cmd_dma, ASPEED_ESPI_OOB_TX_DMA);
		aspeed_espi_oob_write(espi_oob, OOB_TXCMD_DESC_NUM, ASPEED_ESPI_OOB_TX_RING_SIZE);
...
///
	...
	if(espi_oob->dma_mode) {
		if(espi_oob->espi_version == 6) {
			...
			aspeed_espi_oob_write(espi_oob, espi_oob->oob_rx_buff_dma, ASPEED_ESPI_OOB_RX_DMA);
			aspeed_espi_oob_write(espi_oob, espi_oob->oob_tx_buff_dma, ASPEED_ESPI_OOB_TX_DMA);
		}
		espi_oob->oob_rx_full = 0;
