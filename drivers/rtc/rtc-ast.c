/*
 * rtc-ast.c - RTC driver for the Aspeed SoC
 *
...
static const struct rtc_class_ops ast_rtcops = {
        .ioctl            = ast_rtc_ioctl,
...
static int __init ast_rtc_probe(struct platform_device *pdev)
{
	...
	dev_set_drvdata(&pdev->dev, ast_rtc);
	
	pr_debug("base = 0x%p, irq = %d , %s\n", ast_rtc->base, ast_rtc->irq, pdev->name);

	rtc_dev = devm_rtc_device_register(&pdev->dev, pdev->name, 
					&ast_rtcops, THIS_MODULE);
	if (IS_ERR(rtc_dev)) {
			ret = PTR_ERR(rtc_dev);
			goto unmap;
	}
